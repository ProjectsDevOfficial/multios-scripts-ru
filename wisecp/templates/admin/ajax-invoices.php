<?php
    ...
    $visibility_taxed   = Config::get("options/taxation") && Config::get("options/invoice-formalization-status");
    ...
    if(isset($list) && $list){
        foreach($list AS $i=>$row){
            ...
            if($status == "paid" || $status == "taxed" || $status == "untaxed")
                $date_detail = $row["status"] == "paid" ? DateManager::format(Config::get("options/date-format")." - H:i",$row["datepaid"]) : "-";
            ...
            if($visibility_taxed) array_push($item,$taxed_detail);
