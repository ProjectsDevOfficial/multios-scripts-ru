        ...
        <form action="<?php echo $links["controller"]; ?>" method="post" id="settingsForm" enctype="multipart/form-data">
            ...
            <div class="formcon">
                ...
                <div class="yuzde70" id="template-variables">
                    ...
                    <span>{theme_color1}</span>
                    <span>{theme_color2}</span>
                    <span>{theme_text_color}</span>
                </div>
            </div>
...
<div id="create-template-modal" style="display: none;">
    ...
    <form action="<?php echo $links["controller"]; ?>" method="post" id="createTemplateForm" enctype="multipart/form-data">
        <input type="hidden" name="operation" value="create_template">

        <div class="padding20">

            <div class="formcon">
                <div class="yuzde30"><?php echo __("admin/manage-website/list-th-category"); ?></div>
                <div class="yuzde70">
                    <select name="group">
                        <?php
                            if($templates)
                            {
                                ...
                                {
                                    ?>
                                    <option value="<?php echo $template["key"]; ?>"><?php echo $template["name"]; ?></option>
                                    <?php
                                }
                            }
                        ?>
            ...
            <div class="formcon">
                <div class="yuzde30"><?php echo __("admin/notifications/create-template-name"); ?></div>
                <div class="yuzde70">
        ...
        <div class="modal-foot-btn">
            <a id="create-template-submit" href="javascript:void(0);" class="green lbtn"><?php echo ___("needs/button-create"); ?></a>
        </div>

    </form>
    <script type="text/javascript">
        $(document).ready(function(){
            $("#create-template-submit").on("click",function(){
                MioAjaxElement($(this),{
                    waiting_text: '<?php echo ___("needs/button-waiting"); ?>',
                    progress_text: '<?php echo ___("needs/button-uploading"); ?>',
                    result:"createTemplateForm_handler",
                });
            });
        });

        function createTemplateForm_handler(result){
            if(result !== ''){
                var solve = getJson(result);
                if(solve !== false){
                    if(solve.status === "error"){
                        if(solve.for != undefined && solve.for !== ''){
                            $("#createTemplateForm "+solve.for).focus();
                            $("#createTemplateForm "+solve.for).attr("style","border-bottom:2px solid red; color:red;");
                            $("#createTemplateForm "+solve.for).change(function(){
                                $(this).removeAttr("style");
                            });
                        }
                        if(solve.message != undefined && solve.message != '')
    ...
    <div class="icerik-container">
        <div class="icerik">
            ...
            <div class="icerikbaslik">
                ...
                <a href="javascript:open_modal('settings',{width:'800px'});void 0;" class="lbtn" style="margin-right: 7px;"><i class="fa fa-cog"></i> <?php echo __("admin/notifications/templates-settings"); ?></a>

                <a href="javascript:open_modal('create-template-modal',{title:'<?php echo __("admin/notifications/create-template"); ?>',width:'800px'});void 0;" class="lbtn"><i class="fa fa-plus"></i> <?php echo __("admin/notifications/create-template"); ?></a>
                                            ...
                                            foreach($template["items"] AS $key=>$item){
                                                ...
                                                <div class="formcon">
                                                    <div class="yuzde70">
                                                        ...
                                                        <?php echo $name; ?>
                                                        <?php if($custom): ?><strong>[<?php echo __("admin/notifications/custom"); ?>]</strong><?php endif; ?>
                                                    </div>
                                                    <div class="yuzde30">
                                                        <a href="<?php echo $detail_link; ?>" data-tooltip="<?php echo ___("needs/button-edit"); ?>" class="sbtn"><i class="fa fa-edit"></i></a>
                                                        <?php if($custom): ?>
                                                            <a href="javascript:void 0;" onclick="delete_template(this,'<?php echo $template['key']; ?>','<?php echo $key; ?>');" data-tooltip="<?php echo ___("needs/button-delete"); ?>" class="sbtn red"><i class="fa fa-trash"></i></a>
                                                        <?php endif; ?>
                                                    </div>
                                                </div>
                                                <?php
