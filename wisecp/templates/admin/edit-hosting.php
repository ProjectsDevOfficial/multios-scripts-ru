...
<head>
    <?php
        ...
        $server_group_id = isset($module_data["server_group_id"]) ? $module_data["server_group_id"] : (isset($options["server_group_id"]) ? $options["server_group_id"] : false);
                                ...
                                <div class="yuzde70">
                                    <script type="text/javascript">
                                        ...
                                        $(document).ready(function(){
                                            ...
                                            <?php if($server_group_id > 0): ?>
                                                shared_server_group_trigger();
                                            <?php endif; ?>
                                            <?php if($server_id > 0): ?>
                                            ...
                                            $("select[name=server_group_id]").change(shared_server_group_trigger);
                                            $("select[name=server_id]").change(shared_server_trigger);
                                            $('input[name=server_selection]').change(function(){
                                                if($(this).val() === "group")
                                                {
                                                    $("#select-shared-server").css("display","none");
                                                    $("#select-shared-server-group").css("display","block");
                                                }
                                                else if($(this).val() === "server")
                                                {
                                                    $("#select-shared-server-group").css("display","none");
                                                    $("#select-shared-server").css("display","block");
                                                    $("select[name=server_group_id]").val(0).trigger('change');
                                                }
                                            });
                                        });

                                        function shared_server_group_trigger(){
                                            var id          = $("select[name=server_group_id]").val();
                                            var servers     = $("option:selected",$("select[name=server_group_id]")).data("servers");

                                            if(id !== '0' && id !== 0 && id.length > 0)
                                                $('#select-shared-server').val(servers[0]).trigger('change');
                                            else
                                                $("#select-shared-server").val(0).trigger("change");
                                        }
                                        function shared_server_trigger(){
                                            var gid     = $("select[name=server_group_id]").val();
                                    ...
                                    <div class="server_selection_wrap">
                                        <input<?php echo $server_group_id > 0 ? ' checked' : ''; ?> type="radio" name="server_selection" value="group" class="radio-custom" id="server_selection_group">
                                        <label class="radio-custom-label" for="server_selection_group"><?php echo __("admin/products/shared-server-tx19"); ?></label>

                                        <input<?php echo $server_group_id > 0 ? '' : ' checked'; ?> type="radio" name="server_selection" value="server" class="radio-custom" id="server_selection_server">
                                        <label class="radio-custom-label" for="server_selection_server"><?php echo __("admin/products/shared-server-tx33"); ?></label>
                                    </div>

                                    <select name="server_group_id" id="select-shared-server-group" style="<?php echo $server_group_id > 0 ? '' : 'display:none;'; ?>">
                                        <option value="0"><?php echo __("admin/products/shared-server-tx19"); ?></option>
                                        <?php
                                            if(isset($shared_server_groups) && $shared_server_groups)
                                            {
                                                foreach($shared_server_groups AS $sg)
                                                {
                                                    ?>
                                                    <option<?php echo $server_group_id == $sg["id"] ? ' selected' : ''; ?> value="<?php echo $sg["id"]; ?>" data-servers='<?php echo Utility::jencode(explode(",",$sg["servers"])); ?>'><?php echo $sg["name"]; ?></option>
                                                    <?php
                                                }
                                            }
                                        ?>
                                    </select>
                                    <select name="server_id" id="select-shared-server" style="<?php echo $server_group_id > 0 ? 'display:none;' : ''; ?>">
                            ...
                            <script type="text/javascript">
                                ...
                                $(document).ready(function() {
                                    orders = $('#ordersTable').DataTable({
                                        ...
                                        "bProcessing": true,
                                        "bServerSide": true,
                                        "sAjaxSource": "<?php echo Controllers::$init->AdminCRLink("orders"); ?>?operation=ajax-list&from=product&id=<?php echo $product["id"]; ?>&type=hosting",
                                        responsive: true,
