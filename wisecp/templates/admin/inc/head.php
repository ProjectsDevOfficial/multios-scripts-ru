...
<?php endif; ?>
<?php if(in_array("intlTelInput-new",$plugins)): ?>
...
<link rel="stylesheet" href="<?php echo $tadress;?>css/admio-custom.css?v=<?php echo License::get_version(); ?>"/>
<!-- CSS -->
...
<?php if(in_array("tinymce-1",$plugins)): ?>
        ...
        $hook_editor_plugins   = '';
        $hook_editor_toolbar   = '';
        $hooks_editor_setup     = Hook::run("AddWysiwygEditorSetup",$tinymce_options);
        $hooks_editor_plugins   = Hook::run("AddWysiwygEditorPlugins",$tinymce_options);
        $hooks_editor_toolbar   = Hook::run("AddWysiwygEditorToolbar",$tinymce_options);
        if($hook_editor_setup) foreach($hooks_editor_setup AS $hook) $hook_editor_setup .= $hook;
        if($hooks_editor_plugins) foreach($hooks_editor_plugins AS $hook) $hook_editor_plugins .= ' '.$hook;
        if($hooks_editor_toolbar) foreach($hooks_editor_toolbar AS $hook) $hook_editor_toolbar .= ' '.$hook;
            ...
            function tinymce_init(selector){
                ...
                tinymce.init({
                    ...
                    plugins: [
                        ...
                        "insertdatetime media table contextmenu paste imagetools wordcount textcolor colorpicker textpattern<?php echo $hook_editor_plugins; ?>"
                    ],
                    toolbar: "styleselect fontselect fontsizeselect | bold italic underline forecolor backcolor colorpicker | link image | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent | removeformat code<?php echo $hook_editor_toolbar; ?>",
                    content_css: [
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            $('.sitemio-checkbox,.checkbox-custom,.radio-custom').each(function(){
                if($(this).hasClass("sitemio-checkbox"))
                {
                    $(this).removeClass("sitemio-checkbox").addClass("sitemio-checkbox-accessibility");
                    $(this).next('label').removeClass("sitemio-checkbox-label");
                }

                if($(this).hasClass("checkbox-custom"))
                {
                    $(this).removeClass("checkbox-custom").addClass("sitemio-checkbox-accessibility");
                    $(this).next('label').removeClass("checkbox-custom-label");
                }

                if($(this).hasClass("radio-custom"))
                {
                    $(this).removeClass("radio-custom").addClass("sitemio-checkbox-accessibility");
                    $(this).next('label').removeClass("radio-custom-label");
                }
            });
        });
</script>
