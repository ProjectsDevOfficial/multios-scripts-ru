...
<audio hidden id="notice_audio" preload="none">
    ...
    <div style="display: none;" id="server_is_full_modal" data-izimodal-title="<?php echo __("admin/products/shared-server-tx30"); ?>">
        <div class="padding20">
            <p><?php echo __("admin/products/shared-server-tx31"); ?></p>
        </div>
        <div class="modal-foot-btn">
            <a id="server_is_full_ok" href="javascript:void(0);" class="red lbtn"><?php echo ___("needs/ok"); ?></a>
        </div>
    </div>
    ...
    if(Admin::isPrivilege(["HELP_UPDATES_LOOK"])){
        if($new_version = Updates::check_new_version()){
            ...
            if(isset($new_version["status"]) && $new_version["status"] == "error"){
                ...
                $is_beta_update = $new_version["is_beta_update"] ?? false;
                $beta_version   = $is_beta_update ? $new_version["Beta"]["version"] : false;

                $perma_version = Filter::permalink($new_version["version"].($is_beta_update ? "-".$beta_version : ''));
                if(!Cookie::get("version-".$perma_version)){
                    ?>
                    <div class="critical-update-notification" id="version_notification_bar" style="display: none;">
                        <strong><?php echo __("admin/help/version-update-notification-text1",['{version}' => "V".$new_version["version"].(isset($new_version["type"]) && $new_version["type"] == "Beta" ? " ".$new_version["type"].($is_beta_update ? ' V'.$beta_version : '') : '') ]); ?></strong> <?php echo __("admin/help/version-update-notification-text2"); ?>
...
<div style="display:none;font-size:13px;" id="transaction" data-iziModal-title="<?php echo __("admin/index/critical-transaction-notifications"); ?>">
    <div class="padding20">
                    ...
                    <div class="formcon<?php echo !$row["unread"] ? ' unread-event' : ''; ?>" style="<?php echo $row["unread"] ? 'opacity:0.5; filter:alpha(opacity=50);' : ''; ?>"<?php echo !isset($row["non-unread"]) ? ' id="event_'.$row["id"].'"' : ''; ?>>
                        <div class="notificon">
                            <?php
                                ...
                                elseif($row["icon"])
                                {
                                    ?><i class="fa <?php echo $row["icon"]; ?>" aria-hidden="true"></i><?php
                                }
                            ?>
                        </div>
                        ...
                        <div class="notifiright">
                            ...
                            <a style="<?php echo (!$row["unread"] && !isset($row["non-unread"])) ? '' : 'display:none;'; ?>" class="notifibtn read_event_<?php echo $row["id"]; ?>_button" href="javascript:<?php echo $row["name"] == "server-is-full" ? 'read_server_is_full' : 'readEvent'; ?>(<?php echo $row["id"]; ?>);void 0;"><i class="fa fa-angle-double-right" aria-hidden="true"></i> <?php echo __("admin/index/critical-transaction-notifications-3"); ?></a>
        ...
        <script type="text/javascript">
            ...
            function read_server_is_full(id)
            {
                open_modal("server_is_full_modal");
                $("#server_is_full_modal").on("click","#server_is_full_ok",function(){
                    readEvent(id);
                    close_modal("server_is_full_modal");
                });
            }
        </script>
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            check_critical_notifications_bar(true);
            if(document.getElementById("period-warning"))
            {
                setTimeout(function(){
                    $("#period-warning").slideDown(600);
                },2000);
            }

        });

        function check_critical_notifications_bar(first){
            var is_have = false;
            ...
            {
                if(first)
                    setTimeout(function(){ $("#critical_notifications_bar").slideDown(600); },2000);
                else
...
?>
<?php
    $ldata      = FileManager::get_license_file_data();
    $linfo      = $ldata ? ($ldata["info"] ?? []) : [];
    $ldueDate   = $linfo["due-date"] ?? "0000-00-00 00:00:00";

    if(($linfo["trial_license"] ?? 0) && !str_starts_with($ldueDate,'0000'))
    {
        $dueDate       = substr($ldueDate,0,10);
        $rDay          = DateManager::remaining_day($dueDate,DateManager::Now("Y-m-d"));
        if($rDay > 1)
        {
            if($ui_lang == "tr")
            {
                ?>
                <div class="headwarnbar" id="period-warning" style="display: none;"><div class="headwarnbar-con">Deneme sürenizin bitimine <strong><?php echo $rDay; ?> Gün</strong> kaldı. <a class="headwarnbar-btn" target="_blank" href="https://www.wisecp.com/tr/pricing"><i class="fas fa-shopping-cart"></i>Hemen Lisans Edin</a></div></div>
                <?php
            }
            else
            {
                ?>
                <div class="headwarnbar" id="period-warning" style="display: none;"><div class="headwarnbar-con">There are <strong><?php echo $rDay; ?> days</strong> left until your trial period ends. <a class="headwarnbar-btn" target="_blank" href="https://www.wisecp.com/en/pricing"><i class="fas fa-shopping-cart"></i>Get License Now</a></div></div>
                <?php
            }
        }
        else
        {
            ...
            {
                ?>
                <div class="headwarnbar headwarnbar-danger" id="period-warning" style="display: none;"><div class="headwarnbar-con">Deneme süreniz <strong>bugün sona eriyor!</strong> Yönetim paneline erişimi kaybetmek üzeresiniz. <a class="headwarnbar-btn" target="_blank" href="https://www.wisecp.com/tr/pricing"><i class="fas fa-shopping-cart"></i>Hemen Lisans Edin</a></div></div>
                <?php
            }
            else
            {
                ?>
                <div class="headwarnbar headwarnbar-danger" id="period-warning" style="display: none;"><div class="headwarnbar-con">Your trial period <strong>ends today!</strong> You are about to lose access to the admin area. <a class="headwarnbar-btn" target="_blank" href="https://www.wisecp.com/en/pricing"><i class="fas fa-shopping-cart"></i>Get License Now</a></div></div>
                <?php
            }
...
<div class="header">
    ...
    <div class="logo">
        <a href="<?php echo $dashboard_link; ?>"><img src="<?php echo Utility::image_link_determiner("resources/uploads/logo/admin-logo.svg"); ?>?v=3" width="250" height="auto"></a>
    </div>
