...
<head>
    <?php
        ...
        if(isset($user)){
            $user_name  = "<strong>".($ticket["name"] ? $ticket["name"] : $user["full_name"])."</strong>";
    ...
    <script type="text/javascript">
        ...
        var mark_as_custom_data_el;
        var mark_as_custom_data_reply_id;
        ...
        $(document).ready(function(){
            ...
            if(device_mobile){
                ...
                setTimeout(function(){
                    ...
                    var message = $("#admin_signature_mobile").html();
            ...
            $("#mobile_open_reply").click(function(){
                ...
                $("#reply_message").css("visibility","visible");
            });
            ...
            $('#ticketLogs').DataTable({
                ...
                "bProcessing": true,
                "bServerSide": true,
                "sAjaxSource": "<?php echo $links["ajax-ticket-logs"]; ?>",
                responsive: true,
                "oLanguage":<?php include __DIR__.DS."datatable-lang.php"; ?>
            });

            setInterval(function(){
                $('.ticket-message-toolbar').each(function(){
                    let visibleLinksCount = $('a',$(this)).filter(function(){
                ...
                if(result !== ''){
                    ...
                    if(solve !== false){
                        ...
                        if(solve.status !== undefined && !changed_status){
                            ...
                            $("#get_status option[value='"+solve.status+"']").prop("selected",true);
                        }
                ...
                if(solve !== false){
                    if(solve.status == "error"){
                        ...
                        if($("#editForm input[name=operation]").val() === "reply_request")
                        {
                            var value = '';
                            if(device_mobile){
                                var signature = $("#admin_signature").html();
        ...
        function change_predefined_reply(elem) {
            ...
            if (device_mobile) {
                ...
                var signature = $("#admin_signature_mobile").html();
                signature = signature.replace(/<br\s*[\/]?>/gi, '\n');
                value += signature;
            }
            else
            {
        ...
        function editReply(id){
            let wrap_div = $(".reply-item-"+id);

            $(".goruldu",wrap_div).prependTo(".ticket-update-btns",wrap_div);
            $(".goruldu",wrap_div).attr("style","margin-top: 7px;float: left;margin-right: 13px;");
            ...
            $("#editReply_"+id+" .edit-btn-cancel").click(function(){
                ...
                $(".reply-item-" + id + " .goruldu").removeAttr("style").appendTo(".reply-item-" + id + " .reply-message");
        ...
        function mark_as_custom_data(el,reply_id,submit)
        {
            if(submit)
            {
                let request = MioAjax({
                    ...
                    method:"POST",
                    data:{
                        operation:"add_reply_to_custom_data",
                        ticket_id:<?php echo $ticket["id"]; ?>,
                        reply_id: mark_as_custom_data_reply_id
                    }
                },true,true);
                request.done(function(result){
                    if(result !== ''){
                        ...
                        if(solve !== false){
                            ...
                            {
                                $("#custom_data_textarea").val(solve.message);
            ...
            {
                mark_as_custom_data_el          = el;
                mark_as_custom_data_reply_id    = reply_id;

                open_modal("markAsCustomDataModal");
            }
...
<div id="admin_signature" style="display: none;"><?php echo nl2br($admin_signature); ?></div>
<div id="admin_signature_mobile" style="display: none;"><?php echo $admin_signature; ?></div>
            ...
            <div class="destekdetay">
                <form action="<?php echo $links["controller"]; ?>" method="post" id="editForm" enctype="multipart/form-data">
                    ...
                    <div class="yuzde48">
                                                ...
                                                <optgroup label="<?php echo $group["name"]; ?>">
                                                    <?php
                                                        foreach($group["items"] AS $row){
                                                            if(isset($service) && $service["id"] == $row["id"])
                                                                $selected_service = $row;
                                                            ?>
                                                            <option<?php echo isset($service) && $service["id"] == $row["id"] ? ' selected' : ''; ?> value="<?php echo $row["id"]; ?>"><?php echo ('('.Filter::html_clear($situations_orders[$row["status"]]).') ').$row["name"]; ?></option>
                                                            <?php
                                                        }
                                                    ?>
                                                </optgroup>
                        ...
                        <?php if($selected_service): ?>
                            <div class="formcon couplediv">
                                ...
                                <div class="yuzde70">
                                    <span style="margin-right:10px;"><?php echo stristr($selected_service["duedate"],'1881') ? ___("date/period-none") : DateManager::format(Config::get("options/date-format")." - H:i",$selected_service["duedate"]);?></span>
                                    <div class="listingstatus"><?php echo $situations_orders[$selected_service["status"]]; ?></div>
                                </div>
                            </div>
                        <?php endif; ?>
                        ...
                        <div class="formcon">
                            ...
                            <div class="yuzde70">
                                <select<?php echo $privOperation ? '' : ' disabled'; ?> name="status_x" id="get_status" onchange="change_status();">
                                    <?php
                                        foreach(array_merge(__("admin/tickets/situations"),(($custom_statuses ?? []) ? $custom_statuses : [])) AS $key=>$row){
                                            $selected_status    = $ticket["cstatus"] ? $ticket["cstatus"] : $ticket["status"];
                                            if(!is_int($key) && ($key == 'active' || $key == 'inactive')) continue;
                                            if(is_int($key))
                                            {
                                                $key        = $row["type"]."-".$row["id"];
                                                $selected   = $selected_status == $row["id"];
                                                $row        = $row["languages"][$ui_lang]["name"];
                                            }
                                            else
                                                $selected = $selected_status == $key;
                                            ?>
                                            <option<?php echo $selected ? ' selected' : ''; ?> value="<?php echo $key; ?>"><?php echo $row; ?></option>
                                            <?php
                                        }
                                    ?>
                                </select>
                    ...
                    <div class="yuzde50">
                        ...
                        <div id="tab-tab">
                            <ul class="tab">
                                <li><a href="javascript:void(0)" class="tablinks" onclick="open_tab(this, 'reply','tab')" data-tab="reply"><?php echo __("admin/tickets/tab-reply"); ?></a></li>
                                <li><a href="javascript:void(0)" class="tablinks" onclick="open_tab(this, 'log','tab')" data-tab="log"><?php echo __("admin/tickets/tab-log"); ?></a></li>
                            </ul>

                            <!-- tab reply start -->
                            <div class="tabcontent" id="tab-reply">
                                ...
                                <div id="reply_wrap">
                                    <?php
                                        if($privOperation){
                                            ...
                                            <div class="formcon" id="clientInfo">
                                            ...
                                            <?php if($ticket["email"]): ?>
                                                <div class="formcon" id="ticket-email">
                                                    <div class="yuzde30"><?php echo __("admin/ac-settings/email"); ?></div>
                                                    <div class="yuzde70"><?php echo $ticket["email"]; ?></div>
                                                </div>
                                            <?php endif; ?>

                                            <?php if($ticket["phone"] ?? ''): ?>
                                                <div class="formcon" id="ticket-phone">
                                                    <div class="yuzde30"><?php echo __("admin/admins/form-field-gsm"); ?></div>
                                                    <div class="yuzde70"><?php echo "+".$ticket["phone"]; ?></div>
                                                </div>
                                            <?php endif; ?>

                                            <?php if($ticket["ip"] ?? ''): ?>
                                                <div class="formcon" id="ticket-ip">
                                                    <div class="yuzde30"><?php echo __("admin/manage-website/messages-ip"); ?></div>
                                                    <div class="yuzde70"><?php echo $ticket["ip"]; ?></div>
                                                </div>
                                            <?php endif; ?>



                                            <div class="formcon" id="predefined-reply-wrap">
                                        ...
                                        <div id="wrapper">
                                            ...
                                            <div class="mobanswerarea-con">
                                                <div class="mobanswerarea-pad">
                                                    <textarea id="reply_message" class="tinymce-1" rows="10" name="message"><?php echo nl2br($admin_signature); ?></textarea>
                                                </div>
                                            </div>
                                ...
                                <div class="ticketdetailbtns">
                                    <?php
                                        if($ticket["custom_fields"] && sizeof($ticket["custom_fields"]) > 0)
                                        {
                                            ?>
                            ...
                            <div id="tab-log" class="tabcontent">
                                ...
                                <table width="100%" id="ticketLogs" class="table table-striped table-borderedx table-condensed nowrap">
                    ...
                    <div id="ticketCustomFields" style="display: none;" data-izimodal-title="<?php echo __("admin/tickets/request-detail-view-custom-fields"); ?>">
                        <div class="padding20">
                            ...
                            <?php if($ticket["custom_fields"]["private_data"] ?? ''): ?>
                                <div class="ticketcustomitem" id="field-private-data-wrap">
                                    <textarea id="custom_data_textarea" name="fields[private_data]" rows="14" style="font-size: 14px;padding:0px;"><?php echo $ticket["custom_fields"]["private_data"]; ?></textarea>
                                </div>
                            <?php else: ?>
                                <?php
                                        ...
                                        <div class="ticketcustomitem department-<?php echo $field["did"]; ?>-field department-fields" id="field-<?php echo $field["id"]; ?>-wrap" <?php echo $wrap_invisible; ?>>
                                            ...
                                            <div class="yuzde50">
                                                <?php
                                                    ...
                                                    elseif($rtype == "checkbox")
                                                    {
                                                        if($response){
                                                            ...
                                                            foreach($options AS $opt){
                                                                $selected = in_array($opt["id"],$response);
                                                                ?>
                                                                <input class="checkbox-custom" type="checkbox" name="fields[<?php echo $field["id"]; ?>][]" value="<?php echo $opt["id"]; ?>" id="field-checkbox-<?php echo $field["id"]."-".$opt["id"]; ?>"<?php echo $selected ? ' checked' : ''; ?>>
                                                                <label class="checkbox-custom-label" style="margin-right: 5px;" for="field-checkbox-<?php echo $field["id"]."-".$opt["id"]; ?>"><?php echo $opt["name"]; ?></label>
                                                                <?php
                                                            }
                                                            ?>
                                                    ...
                                                    elseif($rtype == "select" || $rtype == "radio")
                                                    {
                                                        ?>
                                                        <select name="fields[<?php echo $field["id"]; ?>]">
                                                            <?php
                                                                if($response){
                                                                    foreach($options AS $opt){
                                                                        $selected = $opt["id"] == $response;
                                                                        ?>
                                                                        <option value="<?php echo $opt["id"]; ?>"<?php echo $selected ? ' selected' : ''; ?>><?php echo $opt["name"]; ?></option>
                                                                        <?php
                                                                    }
                                                                }
                                                            ?>
                        ...
                        <div class="modal-foot-btn">
                            <a href="javascript:void(0);" class="green lbtn" id="edit_request_submit2"><?php echo __("admin/tickets/request-detail-button-update"); ?></a>
                        </div>
                    </div>

                    <div id="markAsCustomDataModal" data-izimodal-title="<?php echo __("admin/tickets/mark-as-custom-data"); ?>" style="display: none;">
                        <div class="padding20">
                            <div class="mark-as-privated-data">
                                <i class="fas fa-shield-alt"></i>

                                <h5><?php echo __("admin/tickets/mark-as-custom-data-modal-text1"); ?></h5>

                                <ol>
                                    <li><?php echo __("admin/tickets/mark-as-custom-data-modal-text2"); ?></li>
                                    <li><?php echo __("admin/tickets/mark-as-custom-data-modal-text3"); ?></li>
                                    <li><?php echo __("admin/tickets/mark-as-custom-data-modal-text4"); ?></li>
                                </ol>
                                <div class="clear"></div>
                            </div>
                        </div>
                        <div class="modal-foot-btn">
                            <a href="javascript:void(0);" class="green lbtn" onclick="mark_as_custom_data(this,0,true);"><?php echo ___("needs/ok"); ?></a>
                        </div>
                    </div>
