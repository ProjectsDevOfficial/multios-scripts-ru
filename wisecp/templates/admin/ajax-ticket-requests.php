<?php
    ...
    if(isset($list) && $list){
        foreach($list AS $i=>$row){
            ...
            if($row["pipe"] && !($row["WChat"] ?? false))
            {
                $subject_detail .= '<div class="clear"></div><span class="piping-ticketlist-tag"><i class="fas fa-envelope"></i> Email Pipe</span>';
            }
            ...
            $perms .= '<a href="'.$detail_link.'" data-tooltip="'.__("admin/tickets/requests-td-detail-button").'" class="sbtn" data-id="'.$id.'" data-details="'.base64_encode(Utility::jencode($row)).'"><i class="fa fa-search"></i></a> ';
