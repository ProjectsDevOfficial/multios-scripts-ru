...
<head>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function(){
            ...
            $('#error-debug-status,#development-status,#error-log-status').change(function(){
                if($(this).prop('checked'))
                {
                    $(this).prop('checked',false);
                    checkbox_id = $(this).attr("id");
                    open_modal('status_modal');
                }
    ...
    <div id="status_modal" style="display: none;" data-izimodal-title="<?php echo __("admin/settings/theme-maintenance-warning"); ?>">
        <div class="padding20">
            <p><?php echo __("admin/tools/log-warning-modal"); ?></p>


            <div class="line"></div>
            <div style="text-align:center;">
                <input type="checkbox" onchange="$('#'+checkbox_id).prop('checked',$(this).prop('checked')); changes();" id="status-modal-warning" class="checkbox-custom">
                <label class="checkbox-custom-label" for="status-modal-warning"><span class=""><?php echo __("admin/settings/theme-maintenance-warning-tx2"); ?></span></label>
            </div>
        </div>
        <div class="modal-foot-btn">
            <a onclick="close_modal('status_modal');" href="javascript:void(0);" class="red lbtn"><?php echo ___("needs/ok"); ?></a>
        </div>
    </div>
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            ...
            $("#error-log-status,#error-debug-status,#development-status").on("change",changes);
        });
