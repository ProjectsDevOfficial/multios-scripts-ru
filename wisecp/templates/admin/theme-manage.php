<?php
    ...
    $upload_max_filesize_suggested_limit = 20 * 1024 * 1024;
    $upload_max_filesize_error           = Bootstrap::$lang->get_cm("admin/settings/error11",['{value}' => $upload_max_filesize_1_raw]);
?>
<!DOCTYPE html>
<html>
<head>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function(){
            ...
            $("#upload_theme_file_btn").click(function(){
                ...
                <?php if($upload_max_filesize_1_raw != "-1" && $upload_max_filesize_1_raw): ?>
                if(size > <?php echo $upload_max_filesize_1; ?>)
                {
                    alert_error("<?php echo trim(str_replace('"','\"',$upload_max_filesize_error)); ?>",{timer:5000});
                    return false;
                }
                <?php endif; ?>
            ...
            $('#maintenance-mode').change(function(){
                if($(this).prop('checked'))
                {
                    $(this).prop('checked',false);
                    open_modal('maintenance_modal');
                }
            });

            $("input[name=header_logo]").change(function(){
                setTimeout(async function(){
                    let colors = await get_image_dominant_colors("header_logo_preview");
                    if(colors[0] !== undefined) $("input[name=new_color1]").val(colors[0]);
                    if(colors[1] !== undefined) $("input[name=new_color2]").val(colors[1]);
                },1000);
            });
        });
        ...
        //header_logo_preview
        async function get_image_dominant_colors(id) {
            ...
            try {
                return await analyzeColors(dataURL);
            } catch (error) {
                console.log(error);
                return [];
            }
        ...
        function analyzeColors(dataURL) {
            return new Promise((resolve, reject) => {
                ...
                image.onload = function() {
                    ...
                    const colors = kmeans(pixelColors, 2);

                    let mostVibrantColor = colors[0];
                    const totalColorValue = colors.map(color => color.reduce((sum, val) => sum + val, 0));

                    if (totalColorValue[1] > totalColorValue[0]) {
                        mostVibrantColor = colors[1];
                    }
                    ...
                    if (darkColor && resultColors.length > 0) {
                        const darkerTone = darkenColor(resultColors[0], 80);
                        resultColors.push(darkerTone);
                    }

                    let colorResults = [];

                    resultColors.forEach(color => {
                        if (color !== mostVibrantColor)
                        {
                            let [r, g, b] = color;
                            let hexColor = `#${toHex(r)}${toHex(g)}${toHex(b)}`;
                            colorResults.push(hexColor);
                        }
                    });

                    let [r, g, b] = mostVibrantColor;
                    let hexColor = `#${toHex(r)}${toHex(g)}${toHex(b)}`;

                    colorResults.push(hexColor);

                    resolve(colorResults);
                };
        ...
        function kmeans(data, k) {
            ...
            for (let i = 0; i < maxIterations; i++) {
                ...
                data.forEach(datum => {
                    const distances = centroids.map(centroid => euclideanDistance(datum, centroid));
                    const clusterIndex = distances.indexOf(Math.min(...distances));
                    clusters[clusterIndex].push(datum);
                });

                const newCentroids = clusters.map(cluster => {
                    if (cluster.length > 0) {
                        return [
                            average(cluster.map(c => c[0])),
                            average(cluster.map(c => c[1])),
                            average(cluster.map(c => c[2]))
                        ];
                    } else {
                ...
                if (centroids.every((c, i) => c.every((val, j) => val === newCentroids[i][j]))) {
                    break;
                }
        ...
        function darkenColor(color, amount) {
            return color.map(val => Math.max(0, val - amount));
        }
    </script>
...
<div id="maintenance_modal" style="display: none;" data-izimodal-title="<?php echo __("admin/settings/theme-maintenance-warning"); ?>">
    <div class="padding20">
        <p><?php echo __("admin/settings/theme-maintenance-warning-tx1"); ?></p>


        <div class="line"></div>
        <div style="text-align:center;">
        <input type="checkbox" onchange="$('#maintenance-mode').prop('checked',$(this).prop('checked'));" id="maintenance-mode-warning" class="checkbox-custom">
        <label class="checkbox-custom-label" for="maintenance-mode-warning"><span class=""><?php echo __("admin/settings/theme-maintenance-warning-tx2"); ?></span></label>
        </div>

    </div>
    <div class="modal-foot-btn">
        <a onclick="close_modal('maintenance_modal');" href="javascript:void(0);" class="red lbtn"><?php echo ___("needs/ok"); ?></a>
    </div>
</div>
...
<div id="theme_settings_modal" data-izimodal-title="" style="display: none;"></div>
                ...
                <div id="group-logo" class="tabcontent"><!-- tab content start -->
                    ...
                    <div class="adminpagecon">
                        ...
                        <form action="<?php echo $links["controller"]; ?>" method="post" enctype="multipart/form-data" id="form1">
                            ...
                            <input type="hidden" name="new_color1" value="">
                            <input type="hidden" name="new_color2" value="">

                                <div class="logos-icons">
                                    <div class="formcon">
                                        <div class="yuzde30">
                                            <?php echo __("admin/settings/header-logo-upload"); ?>
                                        </div>
                                        <div class="yuzde70">
                                            <input name="header_logo" type="file" onchange="read_image_file(this,'header_logo_preview');" accept=".jpg, .jpeg, .png, .gif, .svg, .webp">
                                           <img style="" src="<?php echo $header_logo_link; ?>?time=<?php echo time(); ?>" width="150" height="auto" id="header_logo_preview">
                                        </div>
                                    </div>
                                    <div class="formcon">
                                        <div class="yuzde30">
                                            <?php echo __("admin/settings/footer-logo-upload"); ?>
                                        </div>
                                        <div class="yuzde70">
                                            <input name="footer_logo" type="file" onchange="read_image_file(this,'footer_logo_preview');" accept=".jpg, .jpeg, .png, .gif, .svg, .webp">
                                           <img src="<?php echo $footer_logo_link; ?>?time=<?php echo time(); ?>" width="150" height="auto" id="footer_logo_preview">
                                        </div>
                                    </div>
                                    <div class="formcon">
                                        <div class="yuzde30">
                                            <?php echo __("admin/settings/clientArea-logo-upload"); ?>
                                        </div>
                                        <div class="yuzde70">
                                            <input name="clientArea_logo" type="file" onchange="read_image_file(this,'clientArea_logo_preview');" accept=".jpg, .jpeg, .png, .gif, .svg, .webp">
                                            <img src="<?php echo $clientArea_logo_link; ?>?time=<?php echo time(); ?>" width="150" height="auto" id="clientArea_logo_preview">
                                        </div>
                                    </div>

                                    <div class="formcon">
                                        <div class="yuzde30">
                                            <?php echo __("admin/settings/invoice-detail-logo-upload"); ?>
                                        </div>
                                        <div class="yuzde70">
                                            <input name="invoice_detail_logo" type="file" onchange="read_image_file(this,'invoice_detail_logo_preview');" accept=".jpg, .jpeg, .png, .gif, .svg, .webp">
                                            <img style="" src="<?php echo $invoice_detail_logo_link; ?>?time=<?php echo time(); ?>" width="150" height="auto" id="invoice_detail_logo_preview">
                                        </div>
                                    </div>

                                    <div class="formcon">
                                        <div class="yuzde30">
                                            <?php echo __("admin/settings/favicon-logo-upload"); ?>
                                        </div>
                                        <div class="yuzde70">
                                            <input name="favicon_logo" type="file" onchange="read_image_file(this,'favicon_logo_preview');" accept=".jpg, .jpeg, .png, .gif, .svg, .webp, .ico">
                                            <img src="<?php echo $favicon_link; ?>?time=<?php echo time(); ?>" width="15" height="15" id="favicon_logo_preview">
                                        </div>
                                    </div>
                                                        ...
                                                        <li data-lang="<?php echo $lkey; ?>">
                                                            ...
                                                            <div class="yuzde20" style="text-align: right;">
                                                                <?php if(isset($set_opt) && $set_opt): ?>
                                                                    ...
                                                                    <?php else: ?>
                                                                        <a class="lbtn" href="javascript:open_modalx('block_options_<?php echo $lkey."_".$key; ?>',{title:'<?php echo $name; ?>'});void 0;"><?php echo __("admin/settings/set-options"); ?></a>
                                                                    <?php endif; ?>
                                                                <?php endif; ?>
...
<?php
    if(isset($blocks_modal) && $blocks_modal){
        ...
        ?>
        <script type="text/javascript">
            function open_modalx(id,opt)
            {
                if(!in_array(id,temp_modal))
                {
                    temp_modal.push(id);
                    $("#"+id).iziModal(get_modal_options_generate(opt));
                    $("#"+id).iziModal('open');
                    ...
                    {
                        $exp        = explode("_",$modal_id);
                        $exp_l      = $exp[0] ?? "en";

                        if($block["owner"] == "hosting" || $block["owner"] == "server")
                        {
                        ?>
                            if('block_options_<?php echo $modal_id; ?>' == id)
                            {
                                select2_element = $('#block_options_<?php echo $modal_id; ?> .get-categories-<?php echo $block["owner"]; ?>');
                        ...
                        if($block["owner"] == "product-group")
                        {
                            ?>
                            if('block_options_<?php echo $modal_id; ?>' == id)
                            {
                                select2_element = $('#block_options_<?php echo $modal_id; ?> .get-categories-<?php echo $block["id"]; ?>');
                                select2_element.select2({
                                    width:'100%',
                                    ajax: {
                                        url: '<?php echo $links["get-group-categories-json"]; ?>',
                        ...
                        if($block["key"] == "home-softwares")
                        {
                            ?>
                            if('block_options_<?php echo $modal_id; ?>' == id)
                            {
                                select2_element = $('#block_options_<?php echo $modal_id; ?> #get-softwares-con-<?php echo $block["lang"]; ?> .get-softwares');
                                select2_element.select2({
                                    width: '100%',
                                    ajax: {
                        ...
                        if($block["key"] == "statistics-by-numbers")
                    {
                    ?>
                    if('block_options_<?php echo $modal_id; ?>' == id)
                    {
                        ...
                        <?php if($block["key"] == "about-us" || $block["key"] == "features" || $block["key"] == "news-articles" || $block["key"] == "home-softwares" || $block["key"] == "group-hosting" || $block["key"] == "group-server" || $block["key"] == "group-sms" || $block["owner"] == "product-group"): ?>
                            <div class="formcon">
                                ...
                                <div class="yuzde70">
                                    ...
                                    <?php if($block["pic_id"] != 0): ?>
                                        <div class="clear"></div>
                                        <input type="checkbox" name="picture_hide" value="1" id="<?php echo $modal_id; ?>_picture_hide" class="checkbox-custom">
                                        <label class="checkbox-custom-label" for="<?php echo $modal_id; ?>_picture_hide"><?php echo __("admin/manage-website/invisible"); ?></label>
                                    <?php endif; ?>
                                </div>
                            </div>
                        <?php endif; ?>
