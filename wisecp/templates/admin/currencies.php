...
<head>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $("#EditCurrency").on("click","#editForm_submit",function(){
                MioAjaxElement($(this),{
                    waiting_text: '<?php echo ___("needs/button-waiting"); ?>',
                    progress_text: '<?php echo ___("needs/button-uploading"); ?>',
            ...
            $("#EditCurrency").iziModal(get_modal_options_generate());

            $("#acceptable_modules").select2({width:'100%'});
        ...
        function EditCurrency(id,name){
            ...
            let title   = "<?php echo __("admin/financial/edit-currency")?>";
            title       = title.replace("{name}",name);

            $("#EditCurrency").iziModal('setTitle',title);
            $("#EditCurrency").iziModal('startLoading');
            $("#EditCurrency").iziModal('open');
            ...
            request.done(function(result){
                $("#EditCurrency").iziModal('stopLoading');
                if(result != ''){
                    var solve = getJson(result);
                    if(solve !== false){
                        if(solve.status == "error"){
                        ...
                        else if(solve.status == "successful")
                        {
                            ...
                            $("select[name='format']").val(solve.data.format);
                            ...
                            $("#acceptable_modules option").prop("selected",false);
                            $("#acceptable_modules").trigger("change");

                            if(solve.data.modules !== null && solve.data.modules.length > 0)
                            {
                                let acceptable_modules = solve.data.modules.split(",");

                                $(acceptable_modules).each(function(k,v){
                                    $("#acceptable_modules option[value="+v+"]").prop('selected',true);
                                });
                                $("#acceptable_modules").trigger("change");
                            }
...
<div id="EditCurrency" style="display: none;">
    <div class="padding20">
        <form action="<?php echo $links["controller"]; ?>" method="post" id="EditForm">
            ...
            <div class="formcon">
                <div class="yuzde30"><?php echo __("admin/financial/currency-format"); ?></div>
                <div class="yuzde70">
                    <select name="format" id="format">
                        <option value="0"><?php echo ___("needs/default"); ?></option>
                        <?php
                            $formats = [
                                1   => '1234.56',
                                2   => '1,234.56',
                            ...
                            foreach($formats AS $f_k => $f)
                            {
                                ?>
                                <option value="<?php echo $f_k; ?>"><?php echo $f; ?></option>
                                <?php
                            }
                        ?>
                    </select>
            ...
            <div class="formcon" style="<?php echo version_compare(License::get_version(),'3.1.9','>') ? '' : 'display: none;'; ?>">
                <div class="yuzde30"><?php echo __("admin/financial/acceptable-modules"); ?></div>
                <div class="yuzde70">
                    <select name="modules[]" multiple id="acceptable_modules">
                        <?php
                            if($paymentModules)
                            {
                                foreach($paymentModules AS $mk => $m)
                                {
                                    $mn       = $m["config"]["meta"]["name"] ?? '';
                                    if(isset($m["lang"]["name"])) $mn = $m["lang"]["name"];
                                    ?>
                                    <option value="<?php echo $mk; ?>"><?php echo $mn; ?></option>
                                    <?php
                                }
                            }
                        ?>
                    </select>
                    <span class="kinfo"><?php echo __("admin/financial/acceptable-modules-desc"); ?></span>
                </div>
            </div>
                    ...
                    if(isset($list) && $list){
                        ...
                        foreach($list AS $row){
                            ...
                            <tr id="row_<?php echo $row["id"]; ?>" class="<?php echo $row["status"] != "active" ? 'currency-inactive' : ''; ?>">
                                ...
                                <td align="center">
                                    <a href="javascript:EditCurrency(<?php echo $row["id"]; ?>,'<?php echo $row["name"]; ?>');void 0;" class="sbtn"><i class="fa fa-edit"></i></a>
                                </td>
                            </tr>
                            <?php
                        }
