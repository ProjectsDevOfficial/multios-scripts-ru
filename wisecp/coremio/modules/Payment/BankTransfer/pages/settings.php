...
<form action="<?php echo Controllers::$init->getData("links")["controller"]; ?>" method="post" id="BankTransfer" enctype="multipart/form-data">
    ...
    <div class="formcon">
        <div class="yuzde30"><?php echo $LANG["text12"] ?? 'Payment Notification Form'; ?></div>
        <div class="yuzde70">
            <input<?php echo ($CONFIG["settings"]["pnf"] ?? false) ? ' checked' : ''; ?> type="checkbox" name="pnf" value="1" id="pnf_checkbox" class="sitemio-checkbox">
            <label for="pnf_checkbox" class="sitemio-checkbox-label"></label>
            <div class="clear"></div>
            <span class="kinfo"><?php echo $LANG["text13"] ?? 'Let it be notified to which bank account number the payment was sent and with which sender info the payment was made. (If enabled, no invoice and order will be created before the notification form is sent.)'; ?></span>
