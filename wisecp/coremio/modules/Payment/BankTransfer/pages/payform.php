<?php
    $pnf            = ($module->config["settings"]["pnf"] ?? false);
    $callback       = Controllers::$init->CRLink("payment",["BankTransfer",$module->get_auth_token(),"callback"]);
?>
 <div class="red-info" style="text-align:center;">
    <div class="padding20">
        <h5>
            <?php
                $content_x      = is_array(($_LANG["note"] ?? false)) ? $_LANG["note"]["content"] ?? '' : $_LANG["note"];
                echo Utility::text_replace($content_x,['{code}' => $module->get_rce()]);
                if($pnf) echo $_LANG["text14"];
            ?>
        </h5>
    </div>
 </div>
...
<?php
    if(!$pnf)
    {
        ...
        <script type="text/javascript">
            setTimeout(function(){
                MioAjax({
                    action:"<?php echo $module->links["callback"]; ?>",
                    method:"GET"
                },true,true);
            },5000);
        </script>
...
?>
 ...
 <div class="bank-notification" style="<?php echo $pnf ? '' : 'display:none;'; ?>">
