<?php
    class Ipara extends PaymentGatewayModule
    {
        ...
        public function bin_check($bin_number = "")
        {
            ...
            $request = $this->getiParaOptions($bin_number);

            if($request->result == 0)
            {
                $this->error = $request->errorMessage;
                return false;
            }
            ...
            if(isset($providers[$request->serviceProvider]))
                $service_provider = $providers[$request->serviceProvider];

            return [
                'country'       => 'US',
                'card_type'     => $request->type == 2 ? 'debit' : "credit",
                'schema'        => $service_provider,
                'bank_name'     => $request->bankName,
                'brand'         => $request->cardFamilyName,
            ];
