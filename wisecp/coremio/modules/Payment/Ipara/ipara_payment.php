...
class iParaPayment
{
    ...
    public function payThreeD()
    {
        ...
        $output .= "<html>";
        $output .= "<body>";
        $output .= "<form action=\"" . $this->three_d_url . "\" method=\"post\" id=\"three_d_form\"/>";
        $output .= "<input type=\"hidden\" name=\"orderId\" value=\"" . $this->order_id . "\"/>";
        $output .= "<input type=\"hidden\" name=\"amount\" value=\"" . number_format((float)$this->amount, 2, '', '') . "\"/>";
        $output .= "<input type=\"hidden\" name=\"cardOwnerName\" value=\"" . $this->card['owner_name'] . "\"/>";
        $output .= "<input type=\"hidden\" name=\"cardNumber\" value=\"" . $this->card['number'] . "\"/>";
        $output .= "<input type=\"hidden\" name=\"cardExpireMonth\" value=\"" . $this->card['expire_month'] . "\"/>";
        $output .= "<input type=\"hidden\" name=\"cardExpireYear\" value=\"" . $this->card['expire_year'] . "\"/>";
        $output .= "<input type=\"hidden\" name=\"installment\" value=\"" . $this->installment . "\"/>";
        $output .= "<input type=\"hidden\" name=\"cardCvc\" value=\"" . $this->card['cvc'] . "\"/>";
        $output .= "<input type=\"hidden\" name=\"mode\" value=\"" . $this->mode . "\"/>";
        $output .= "<input type=\"hidden\" name=\"purchaserName\" value=\"" . $this->purchaser['name'] . "\"/>";
        $output .= "<input type=\"hidden\" name=\"purchaserSurname\" value=\"" . $this->purchaser['surname'] . "\"/>";
        $output .= "<input type=\"hidden\" name=\"purchaserEmail\" value=\"" . $this->purchaser['email'] . "\"/>";
        $output .= "<input type=\"hidden\" name=\"successUrl\" value=\"" . $this->success_url . "\"/>";
        $output .= "<input type=\"hidden\" name=\"failureUrl\" value=\"" . $this->failure_url . "\"/>";
        $output .= "<input type=\"hidden\" name=\"echo\" value=\"" . $this->echo . "\"/>";
        $output .= "<input type=\"hidden\" name=\"version\" value=\"" . $this->version . "\"/>";
        $output .= "<input type=\"hidden\" name=\"transactionDate\" value=\"" . $timestamp . "\"/>";
        $output .= "<input type=\"hidden\" name=\"token\" value=\"" . $token . "\"/>";
        $output .= "<input type=\"submit\" value=\"Öde\" style=\"display:none;\"/>";
        $output .= "<noscript>";
        $output .= "<br/>";
        $output .= "<br/>";
        $output .= "<center>";
        $output .= "<h1>3D Secure Yönlendirme İşlemi</h1>";
        $output .= "<h2>Javascript internet tarayıcınızda kapatılmış veya desteklenmiyor.<br/></h2>";
        $output .= "<h3>Lütfen banka 3D Secure sayfasına yönlenmek için tıklayınız.</h3>";
        $output .= "<input type=\"submit\" value=\"3D Secure Sayfasına Yönlen\">";
        $output .= "</center>";
        $output .= "</noscript>";
        $output .= "</form>";
        $output .= "</body>";
        $output .= "<script>document.getElementById(\"three_d_form\").submit();</script>";
        $output .= "</html>";
