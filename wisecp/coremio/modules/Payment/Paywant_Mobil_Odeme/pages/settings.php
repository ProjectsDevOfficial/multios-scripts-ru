<?php
    ...
    $notify_url = Controllers::$init->CRLink("payment",['Paywant_Mobil_Odeme',$module->get_auth_token(),'callback'],"none");
?>

<form action="<?php echo Controllers::$init->getData("links")["controller"]; ?>" method="post" id="Paywant_Mobil_Odeme">
    <input type="hidden" name="operation" value="module_controller">
    <input type="hidden" name="module" value="Paywant_Mobil_Odeme">
    ...
    <div style="float:right;" class="guncellebtn yuzde30"><a id="Paywant_Mobil_Odeme_submit" href="javascript:void(0);" class="yesilbtn gonderbtn"><?php echo $LANG["save-button"]; ?></a></div>
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $("#Paywant_Mobil_Odeme_submit").click(function(){
            MioAjaxElement($(this),{
                waiting_text:waiting_text,
                progress_text:progress_text,
                result:"Paywant_Mobil_Odeme_handler",
            });
        });

    });

    function Paywant_Mobil_Odeme_handler(result){
        if(result != ''){
            var solve = getJson(result);
            if(solve !== false){
                if(solve.status == "error"){
                    if(solve.for != undefined && solve.for != ''){
                        $("#Paywant_Mobil_Odeme "+solve.for).focus();
                        $("#Paywant_Mobil_Odeme "+solve.for).attr("style","border-bottom:2px solid red; color:red;");
                        $("#Paywant_Mobil_Odeme "+solve.for).change(function(){
