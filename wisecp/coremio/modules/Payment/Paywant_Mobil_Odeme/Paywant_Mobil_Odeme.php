...
class Paywant_Mobil_Odeme
{
    ...
    public function get_auth_token()
    {
        ...
        $token = md5(Crypt::encode("Paywant_Mobil_Odeme-Auth-Token=" . $syskey, $syskey));
        return $token;
    }
    ...
    public function get_iframe($ok_url, $fail_url)
    {
        ...
        if ($this->cid_convert_code($checkout_data["currency"]) == "TRY")
        {
            ...
            else
            {
                ...
                {
                    ?>					
					<iframe seamless="seamless" style="display:block; width:100%; height:100vh;" frameborder="0" scrolling='yes' src="<?php echo $jsonDecode->message ?>" id='odemeFrame'></iframe>					<?php
                }
                else
                {
                    echo $response;
