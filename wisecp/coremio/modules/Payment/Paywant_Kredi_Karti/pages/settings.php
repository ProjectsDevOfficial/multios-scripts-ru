<?php
    ...
    $notify_url = Controllers::$init->CRLink("payment",['Paywant_Kredi_Karti',$module->get_auth_token(),'callback'],"none");
?>

<form action="<?php echo Controllers::$init->getData("links")["controller"]; ?>" method="post" id="Paywant_Kredi_Karti">
    <input type="hidden" name="operation" value="module_controller">
    <input type="hidden" name="module" value="Paywant_Kredi_Karti">
    ...
    <div style="float:right;" class="guncellebtn yuzde30"><a id="Paywant_Kredi_Karti_submit" href="javascript:void(0);" class="yesilbtn gonderbtn"><?php echo $LANG["save-button"]; ?></a></div>
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $("#Paywant_Kredi_Karti_submit").click(function(){
            MioAjaxElement($(this),{
                waiting_text:waiting_text,
                progress_text:progress_text,
                result:"Paywant_Kredi_Karti_handler",
            });
        });

    });

    function Paywant_Kredi_Karti_handler(result){
        if(result != ''){
            var solve = getJson(result);
            if(solve !== false){
                if(solve.status == "error"){
                    if(solve.for != undefined && solve.for != ''){
                        $("#Paywant_Kredi_Karti "+solve.for).focus();
                        $("#Paywant_Kredi_Karti "+solve.for).attr("style","border-bottom:2px solid red; color:red;");
                        $("#Paywant_Kredi_Karti "+solve.for).change(function(){
