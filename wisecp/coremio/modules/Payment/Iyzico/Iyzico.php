<?php
    class Iyzico {
        ...
        public $payform=false,$checkout_id=0,$checkout;
        ...
        public function inline_form(){
            ...
            $user_ip                = substr($this->get_ip(),0,50);
            $email                  = $user_data["email"];
            $payment_amount         = round($checkout_data["total"],2);
            $user_basket            = "";
            ...
            $buyer->setIdentityNumber(isset($user_data["identity"]) && $user_data["identity"] ? $user_data["identity"] : "11111111111");
            ...
            if($checkout_items)
            {
                ...
                $sizeOf         = sizeof($items);
                $split          = round($payment_amount / $sizeOf,4);

                foreach($items AS $i)
                {
                    $_amount = $split;
                    if($_amount < 0.00) continue;

                    $category1_name = $i["options"]["category"] ?? 'None';
                    if(!$category1_name) $category1_name = 'None';
                    $firstBasketItem = new \Iyzipay\Model\BasketItem();
                    $firstBasketItem->setId("B".$i["id"]);
                    $firstBasketItem->setName($i["name"]);
                    $firstBasketItem->setCategory1($category1_name);
                    $firstBasketItem->setItemType(\Iyzipay\Model\BasketItemType::VIRTUAL);
                    $firstBasketItem->setPrice($_amount);
                    $basketItems[] = $firstBasketItem;
                }

                $ks                 = array_keys($basketItems);
                $endk               = end($ks);
                $diff_amount        = round($payment_amount - ($split * $sizeOf),4);
                $apply_diff         = false;


                if($diff_amount > 0.0000 || $diff_amount < 0.0000) $apply_diff = true;

                if($apply_diff)
                {
                    $new_amount         = $basketItems[$endk]->getPrice() + ($diff_amount);
                    $basketItems[$endk]->setPrice($new_amount);
                }
            }
