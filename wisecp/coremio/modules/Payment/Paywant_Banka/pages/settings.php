<?php
    ...
    $notify_url = Controllers::$init->CRLink("payment",['Paywant_Banka',$module->get_auth_token(),'callback'],"none");
?>

<form action="<?php echo Controllers::$init->getData("links")["controller"]; ?>" method="post" id="Paywant_Banka">
    <input type="hidden" name="operation" value="module_controller">
    <input type="hidden" name="module" value="Paywant_Banka">
    ...
    <div style="float:right;" class="guncellebtn yuzde30"><a id="Paywant_Banka_submit" href="javascript:void(0);" class="yesilbtn gonderbtn"><?php echo $LANG["save-button"]; ?></a></div>
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $("#Paywant_Banka_submit").click(function(){
            MioAjaxElement($(this),{
                waiting_text:waiting_text,
                progress_text:progress_text,
                result:"Paywant_Banka_handler",
            });
        });

    });

    function Paywant_Banka_handler(result){
        if(result != ''){
            var solve = getJson(result);
            if(solve !== false){
                if(solve.status == "error"){
                    if(solve.for != undefined && solve.for != ''){
                        $("#Paywant_Banka "+solve.for).focus();
                        $("#Paywant_Banka "+solve.for).attr("style","border-bottom:2px solid red; color:red;");
                        $("#Paywant_Banka "+solve.for).change(function(){
