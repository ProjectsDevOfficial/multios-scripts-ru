<?php
    ...
    $additional_sans_count  = (int) $module->get_additional_sans_count();
    $included_sans_count    = (int) $creation_info["included_sans"] ?? 0;
    ...
    $webserver_type = isset($module_data["webserver_type"]) ? $module_data["webserver_type"] : "-1";
    $get_webservers = $module->get_webservers();


    if(isset($creation_info["disable_dcv_m"]) && $creation_info["disable_dcv_m"])
        foreach($creation_info["disable_dcv_m"] AS $m) unset($dcv_methods[strtolower($m)]);
...
<div class="accordion">
    ...
    <div>
        ...
        <div class="formcon">
            <div class="yuzde30">Webserver Type</div>
            <div class="yuzde70">
                <select name="creation_info[webserver_type]">
                    <?php
                        foreach($get_webservers AS $group => $group_servers)
                        {
                            echo '<optgroup label="'.($group == 2 ? 'Geotrust/Symantec/Thawte/RapidSSL' : 'Comodo/GGSSL').'">';
                            foreach($group_servers AS $k => $v)
                            {
                                ?>
                                <option<?php echo $webserver_type == $k ? ' selected' : ''; ?> value="<?php echo $k; ?>"><?php echo $v; ?></option>
                                <?php
                            }
                            echo '</optgroup>';
                        }
        ...
        <?php
            if($established){
                ...
                ?>
                    ...
                    <div class="formcon">
                        <div class="yuzde30">OrderID</div>
                        <div class="yuzde70">
                            <input type="text" style="width: 10%;" value="<?php echo $config["order_id"]; ?>" name="config[order_id]">
                        </div>
                    </div>
