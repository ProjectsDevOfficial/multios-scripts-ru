<?php
    class OztekSMSLib {
        ...
        public function __construct($username_id,$username, $password,$ozelkod) {
            $this->username_id = $username_id;
            $this->username = $username;
            $this->password = $password;
            $this->ozelkod = $ozelkod;
            $this->url 	  = "http://www.oztekbayi.com/panel/";
        }
...
<tur>Normal</tur>
        ...
        public function Balance(){
            ...
            if($this->ozelkod) $params['kno'] = $this->username_id;

            $string = $this->curl_use($this->url.'kullanicibilgi.php?'.http_build_query($params));
            $string = mb_convert_encoding($string, "UTF-8", "ISO-8859-9");
            if(!$string)
            {
                $this->error = 'API tarafından yanıt gelmedi.';
                return false;
            }
            $unitPrice = 0;
            $remainingBalance = 0;
            if (preg_match('/SMS Birim Fiyatı=(\d+(\.\d+)?)/', $string, $matches)) $unitPrice = (float) $matches[1];
            if (preg_match('/Kalan Bakiye=(\d+(\.\d+)?)/', $string, $matches)) $remainingBalance = (float) $matches[1];
            if($unitPrice === 0)
            {
                $this->error = "Birim fiyat sıfır veya tanımlanmamış.";
                return false;
            }
            return intval($remainingBalance / $unitPrice);
        }
