<?php
    ...
    class Netgsm
    {
        ...
        {
            if(!class_exists("NetgsmLibrary")) include __DIR__.DS."Source".DS."class.php";
            ...
            $title              = $config["origin"];
            $this->instance = new NetgsmLibrary($username,$password);
            $this->title = $title;
        }
        ...
        public function body($text='',$template=false,$variables=[],$lang='',$user=0){
            ...
            foreach($currencies AS $row){
                ...
                if(!Validation::isEmpty($row["prefix"]) && $row["prefix"] && $row["prefix"] != "TL")
                    $text = Utility::text_replace($text,[$row["prefix"] => $code]);
                elseif(!Validation::isEmpty($row["suffix"]) && $row["suffix"] && $row["suffix"] != "TL")
        ...
        public function getReport($id=0){
            ...
            if($content){
                ...
                $waiting_arr	    = $content["bekleyen"];
                $conducted_arr      = $content["iletilen"];
                $erroneous_arr	    = $content["hatali"];
