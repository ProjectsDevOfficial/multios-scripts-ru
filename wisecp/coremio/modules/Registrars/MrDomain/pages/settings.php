...
function MrDomain_open_tab(elem, tabName){
    var owner = "MrDomain_tab";
...
<div id="MrDomain_tab">
    <ul class="modules-tabs">
        <li><a href="javascript:MrDomain_open_tab(this,'detail');" data-tab="detail" class="modules-tab-item active"><?php echo $LANG["tab-detail"]; ?></a></li>
        <li><a href="javascript:MrDomain_open_tab(this,'import');" data-tab="import" class="modules-tab-item"><?php echo $LANG["tab-import"]; ?></a></li>
    </ul>

    <div id="MrDomain_tab-detail" class="modules-tabs-content" style="display: block">

        <form action="<?php echo Controllers::$init->getData("links")["controller"]; ?>" method="post" id="MrDomainSettings">
            <input type="hidden" name="operation" value="module_controller">
            <input type="hidden" name="module" value="MrDomain">
            ...
            <div class="formcon">
                ...
                <div class="yuzde70">
                    <input<?php echo $CONFIG["settings"]["adp"] ? ' checked' : ''; ?> type="checkbox" name="adp" value="1" id="MrDomain_adp" class="checkbox-custom">
                    <label class="checkbox-custom-label" for="MrDomain_adp">
            ...
            <div class="formcon">
                ...
                <div class="yuzde70">
                    <a class="lbtn" href="javascript:open_modal('MrDomain_import_tld');void 0;"><?php echo $LANG["import-tld-button"]; ?></a>
                    <div class="clear"></div>
            ...
            <div style="float:left;" class="guncellebtn yuzde30"><a id="MrDomain_testConnect" href="javascript:void(0);" class="lbtn"><i class="fa fa-plug" aria-hidden="true"></i> <?php echo $LANG["test-button"]; ?></a></div>

            <div style="float:right;" class="guncellebtn yuzde30"><a id="MrDomain_submit" href="javascript:void(0);" class="yesilbtn gonderbtn"><?php echo $LANG["save-button"]; ?></a></div>

        </form>
        <script type="text/javascript">
            $(document).ready(function(){
                $("#MrDomain_testConnect").click(function(){
                    $("#MrDomainSettings input[name=controller]").val("test_connection");
                    MioAjaxElement($(this),{
                        waiting_text:waiting_text,
                        progress_text:progress_text,
                        result:"MrDomain_handler",
                    });
                });

                $("#MrDomain_submit").click(function(){
                    $("#MrDomainSettings input[name=controller]").val("settings");
                    MioAjaxElement($(this),{
                        waiting_text:waiting_text,
                        progress_text:progress_text,
                        result:"MrDomain_handler",
            ...
            function MrDomain_handler(result){
                if(result != ''){
                    var solve = getJson(result);
                    if(solve !== false){
                        if(solve.status == "error"){
                            if(solve.for != undefined && solve.for != ''){
                                $("#MrDomainSettings "+solve.for).focus();
                                $("#MrDomainSettings "+solve.for).attr("style","border-bottom:2px solid red; color:red;");
                                $("#MrDomainSettings "+solve.for).change(function(){
    ...
    <div id="MrDomain_tab-import" class="modules-tabs-content" style="display: none;">
        ...
        <script type="text/javascript">
            ...
            function MrDomain_import_handler(result){
                if(result != ''){
                    var solve = getJson(result);
                    if(solve !== false){
                        if(solve.status == "error"){
                            if(solve.for != undefined && solve.for != ''){
                                $("#MrDomainImport "+solve.for).focus();
                                $("#MrDomainImport "+solve.for).attr("style","border-bottom:2px solid red; color:red;");
                                $("#MrDomainImport "+solve.for).change(function(){
                                    $(this).removeAttr("style");
                                });
                            }
                            if(solve.message != undefined && solve.message != '')
            ...
            $(document).ready(function(){
                ...
                $("#MrDomain_import_submit").click(function(){
                    MioAjaxElement($(this),{
                        waiting_text:waiting_text,
                        progress_text:progress_text,
                        result:"MrDomain_import_handler",
                    });
                });

                $('#MrDomain_list_domains').DataTable({
                    "columnDefs": [
                        {
                            "targets": [0],
                            "visible":false,
        ...
        <form action="<?php echo Controllers::$init->getData("links")["controller"]; ?>" method="post" id="MrDomainImport">
            <input type="hidden" name="operation" value="module_controller">
            <input type="hidden" name="module" value="MrDomain">
            <input type="hidden" name="controller" value="import">

            <table width="100%" id="MrDomain_list_domains" class="table table-striped table-borderedx table-condensed nowrap">
            ...
            <div class="guncellebtn yuzde20" style="float: right;">
                <a href="javascript:void(0);" id="MrDomain_import_submit" class="gonderbtn mavibtn"><?php echo $LANG["import-button"]; ?></a>
            </div>
...
<div id="MrDomain_import_tld" style="display: none;" data-izimodal-title="<?php echo $LANG["fields"]["import-tld"]; ?>">
    <script type="text/javascript">
        $(document).ready(function(){

            $("#MrDomain_import_tld_submit").on("click",function(){
                var request = MioAjax({
                    button_element:this,
    ...
    <div class="padding20">
        ...
        <div align="center">
            <div class="yuzde50">
                <a class="yesilbtn gonderbtn" href="javascript:void 0;" id="MrDomain_import_tld_submit"><i class="fa fa-check" aria-hidden="true"></i> <?php echo ___("needs/ok"); ?></a>
            </div>
        </div>
