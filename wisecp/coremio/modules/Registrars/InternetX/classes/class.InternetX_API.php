        ...
        class InternetX_API
        {
            ...
            private function createRequest($params)
            {
			 ...
			 Plugin");
                ...
                if($params->code == "0105")
                {
                    $view           = $xml->createElement('view');

                    $children        = $xml->createElement('children', 1);
                    $limit           = $xml->createElement('limit', 9999);
                    $view->appendChild($children);
                    $view->appendChild($limit);
            ...
            private function processRequest($request, $action='')
            {
                ...
                Modules::save_log("Registrars","InternetX",$action,htmlentities($request),htmlentities($response),$result);
            ...
            public function premiumDomainCreate($domain, $period, $contacts, $nameServers, $usePrivacy, $ip, $mx, $className)
            {
                ...
                $task->domain = new stdClass();
                $task->domain->name = $domain;
                $task->domain->extension = new stdClass();
                ...
                $task->domain->techc = $contacts['techc'];
                $task->domain->zonec = $contacts['zonec'];
                $task->domain->nserver = []; // Array olarak başlat
                ...
                if ($usePrivacy == 1) {
                    $task->domain->use_privacy = $usePrivacy;
                }
                $task->reply_to = self::$reply_to;
                $xml = $this->createRequest($task);
                return $this->processRequest($xml, "domain create");
            }
            ...
            public function domainRenew($domain, $payable, $period)
            {
                $task = new stdClass();
                $task->code = "0101003";
                $task->domain = new stdClass(); // Eksik nesne oluşturma
                $task->domain->name = $domain;
            ...
            public function transferOutInfo($domain)
            {
                $task = new stdClass();
                $task->code = "0106001";
                $task->transfer = new stdClass(); // Eksik nesne oluşturma
                $task->transfer->domain = new stdClass(); // Eksik nesne oluşturma
            ...
            public function redirectCreateEmail($source, $target)
            {
                $task = new stdClass();
                $task->code = "0501";
                $task->redirect = new stdClass(); // Eksik nesne oluşturma
            ...
            public function zoneCreate($zoneName, $nameServers)
            {
                $task = new stdClass();
                $task->code = "0201";
                $task->zone = new stdClass(); // Eksik nesne oluşturma
                $task->zone->name = $zoneName;
                $task->zone->main = new stdClass(); // Eksik nesne oluşturma
                $task->zone->main->value = "";
                $task->zone->main->ttl = "";
                $task->zone->ns_action = "complete";
                $task->zone->www_include = "1";
                $task->zone->soa = new stdClass(); // Eksik nesne oluşturma
                ...
                $task->zone->nserver = []; // Array olarak başlat
                foreach ($nameServers as $k => $ns) {
                    $ns = trim($ns);
                    if (empty($ns)) continue;
                    $task->zone->nserver[$k] = new stdClass(); // Her bir nserver için nesne oluştur
            ...
            public function checkPremiumAvability($domain, $action)
            {
                $task = new stdClass();
                $task->code = "0164";
                $task->domain_premium = new stdClass(); // Eksik nesne oluşturma
