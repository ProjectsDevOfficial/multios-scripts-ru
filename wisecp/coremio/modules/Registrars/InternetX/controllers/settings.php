<?php
    ...
    $check_url          = Filter::init("POST/check_url","hclear");
    $check_username     = Filter::init("POST/check_username","hclear");
    $check_password     = Filter::init("POST/check_password","hclear");

    if($check_password && $check_password != "*****") $check_password = Crypt::encode($check_password,Config::get("crypt/system"));
    ...
    if($check_url != $config["settings"]["check_url"])
        $sets["settings"]["check_url"] = $check_url;

    if($check_username != $config["settings"]["check_username"])
        $sets["settings"]["check_username"] = $check_username;

    if($check_password != "*****" && $check_password != $config["settings"]["check_password"])
        $sets["settings"]["check_password"] = $check_password;
