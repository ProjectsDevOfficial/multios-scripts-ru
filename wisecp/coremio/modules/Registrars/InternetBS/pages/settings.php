...
function InternetBS_open_tab(elem, tabName){
    var owner = "InternetBS_tab";
...
<div id="InternetBS_tab">
    <ul class="modules-tabs">
        <li><a href="javascript:InternetBS_open_tab(this,'detail');" data-tab="detail" class="modules-tab-item active"><?php echo $LANG["tab-detail"]; ?></a></li>
        <li><a href="javascript:InternetBS_open_tab(this,'import');" data-tab="import" class="modules-tab-item"><?php echo $LANG["tab-import"]; ?></a></li>
    </ul>

    <div id="InternetBS_tab-detail" class="modules-tabs-content" style="display: block">

        <form action="<?php echo Controllers::$init->getData("links")["controller"]; ?>" method="post" id="InternetBSSettings">
            <input type="hidden" name="operation" value="module_controller">
            <input type="hidden" name="module" value="InternetBS">
            ...
            <div class="formcon">
                ...
                <div class="yuzde70">
                    <input<?php echo $CONFIG["settings"]["adp"] ? ' checked' : ''; ?> type="checkbox" name="adp" value="1" id="InternetBS_adp" class="checkbox-custom">
                    <label class="checkbox-custom-label" for="InternetBS_adp">
            ...
            <div class="formcon">
                ...
                <div class="yuzde70">
                    <a class="lbtn" href="javascript:open_modal('InternetBS_import_tld');void 0;"><?php echo $LANG["import-tld-button"]; ?></a>
                    <div class="clear"></div>
            ...
            <div style="float:left;" class="guncellebtn yuzde30"><a id="InternetBS_testConnect" href="javascript:void(0);" class="lbtn"><i class="fa fa-plug" aria-hidden="true"></i> <?php echo $LANG["test-button"]; ?></a></div>

            <div style="float:right;" class="guncellebtn yuzde30"><a id="InternetBS_submit" href="javascript:void(0);" class="yesilbtn gonderbtn"><?php echo $LANG["save-button"]; ?></a></div>

        </form>
        <script type="text/javascript">
            $(document).ready(function(){
                $("#InternetBS_testConnect").click(function(){
                    $("#InternetBSSettings input[name=controller]").val("test_connection");
                    MioAjaxElement($(this),{
                        waiting_text:waiting_text,
                        progress_text:progress_text,
                        result:"InternetBS_handler",
                    });
                });

                $("#InternetBS_submit").click(function(){
                    $("#InternetBSSettings input[name=controller]").val("settings");
                    MioAjaxElement($(this),{
                        waiting_text:waiting_text,
                        progress_text:progress_text,
                        result:"InternetBS_handler",
            ...
            function InternetBS_handler(result){
                if(result != ''){
                    var solve = getJson(result);
                    if(solve !== false){
                        if(solve.status == "error"){
                            if(solve.for != undefined && solve.for != ''){
                                $("#InternetBSSettings "+solve.for).focus();
                                $("#InternetBSSettings "+solve.for).attr("style","border-bottom:2px solid red; color:red;");
                                $("#InternetBSSettings "+solve.for).change(function(){
    ...
    <div id="InternetBS_tab-import" class="modules-tabs-content" style="display: none;">
        ...
        <script type="text/javascript">
            ...
            function InternetBS_import_handler(result){
                if(result != ''){
                    var solve = getJson(result);
                    if(solve !== false){
                        if(solve.status == "error"){
                            if(solve.for != undefined && solve.for != ''){
                                $("#InternetBSImport "+solve.for).focus();
                                $("#InternetBSImport "+solve.for).attr("style","border-bottom:2px solid red; color:red;");
                                $("#InternetBSImport "+solve.for).change(function(){
                                    $(this).removeAttr("style");
                                });
                            }
                            if(solve.message != undefined && solve.message != '')
            ...
            $(document).ready(function(){
                ...
                $("#InternetBS_import_submit").click(function(){
                    MioAjaxElement($(this),{
                        waiting_text:waiting_text,
                        progress_text:progress_text,
                        result:"InternetBS_import_handler",
                    });
                });

                $('#InternetBS_list_domains').DataTable({
                    "columnDefs": [
                        {
                            "targets": [0],
                            "visible":false,
        ...
        <form action="<?php echo Controllers::$init->getData("links")["controller"]; ?>" method="post" id="InternetBSImport">
            <input type="hidden" name="operation" value="module_controller">
            <input type="hidden" name="module" value="InternetBS">
            <input type="hidden" name="controller" value="import">

            <table width="100%" id="InternetBS_list_domains" class="table table-striped table-borderedx table-condensed nowrap">
            ...
            <div class="guncellebtn yuzde20" style="float: right;">
                <a href="javascript:void(0);" id="InternetBS_import_submit" class="gonderbtn mavibtn"><?php echo $LANG["import-button"]; ?></a>
            </div>
...
<div id="InternetBS_import_tld" style="display: none;" data-izimodal-title="<?php echo $LANG["fields"]["import-tld"]; ?>">
    <script type="text/javascript">
        $(document).ready(function(){

            $("#InternetBS_import_tld_submit").on("click",function(){
                var request = MioAjax({
                    button_element:this,
    ...
    <div class="padding20">
        ...
        <div align="center">
            <div class="yuzde50">
                <a class="yesilbtn gonderbtn" href="javascript:void 0;" id="InternetBS_import_tld_submit"><i class="fa fa-check" aria-hidden="true"></i> <?php echo ___("needs/ok"); ?></a>
            </div>
        </div>
