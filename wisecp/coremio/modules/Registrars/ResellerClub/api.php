<?php
    class ResellerClub_Api {
        ...
        public function addContact($customer_id=0,$data=[],$tld=false){
        ...
        public function modifyTransferLock($order_id=0,$type=false){
        ...
        public function add_dns_record($domain,$type,$host,$value,$ttl=14400)
        {
            $sth = $this->post("dns/manage","add-".$type."-record");
            $this->addParam("domain-name",$domain);
            $this->addParam("host",$host);
            $this->addParam("value",$value);
            $this->addParam("ttl",$ttl);
            return $sth->build();
        }
        public function update_dns_record($domain,$type,$host,$current_value,$new_value,$ttl=14400)
        {
            $sth = $this->post("dns/manage","update-".$type."-record");
            $this->addParam("domain-name",$domain);
            $this->addParam("host",$host);
            $this->addParam("current-value",$current_value);
            $this->addParam("new-value",$new_value);
            $this->addParam("ttl",$ttl);
            return $sth->build();
        }
        public function delete_dns_record($domain,$type,$host,$value)
        {
            $sth = $this->post("dns/manage","delete-".$type."-record");
            $this->addParam("domain-name",$domain);
            $this->addParam("host",$host);
            $this->addParam("value",$value);
            return $sth->build();
