<?php
    class ResellerClub {
        ...
        function __construct($external=[]){
            ...
            if(!class_exists("ResellerClub_Api")) include __DIR__.DS."api.php";
            ...
            $this->api =  new ResellerClub_Api($uid,$akey,$tmode);
        }
        ...
        public function register($domain='',$sld='',$tld='',$year=1,$dns=[],$whois=[],$wprivacy=false,$tcode=NULL){
            ...
            if(isset($submit["status"]) && ($submit["status"] == "Success" || $submit["status"] == "STATUS_RP_DOMAIN_REGISTRATION_PENDING")){
                ...
                if($wprivacy) {
                    ...
                    $wprivacy_status = true;
                    $wprivacy_message = NULL;
                    $apply = $this->api->purchasePrivacy($order_id);
                    if(!$apply){
                        $wprivacy_message = $this->api->error;
                        $wprivacy_status = false;
                    }

                    if($apply && !(isset($apply["status"]) && $apply["status"] == "Success" && isset($apply["actionstatus"]) && $apply["actionstatus"] == "Success")) {
                        $wprivacy_message = $this->api->error;
                        $wprivacy_status = false;
                    }

                    $rdata["whois_privacy"] = ['status' => $wprivacy_status,'message' => $wprivacy_message];
        ...
        public function domains($test=false,$page_num=1){
            Helper::Load(["User"]);
            $params = [
                'order-by' => "orderid",
                'page-no'  => $page_num,
            ...
            if(sizeof($result) >= 500) $result = array_merge($result,$this->domains($test,$page_num + 1));
        ...
        */
        public function getDnsRecords()
        {
            ...
            foreach($this->config["settings"]["dns-record-types"] ?? [] AS $type)
            {
                $getResult = $this->api->search_dns_records($domain,$type);
                if($getResult && ($getResult["recsindb"] ?? 0) > 0)
                {
                    for($i=1;$i<=$getResult["recsindb"]; $i++)
                    {
                        $recordData = $getResult[$i] ?? [];
                        if($recordData)
                        {
                            $result[] = [
                                'type'      => $type,
        ...
        public function addDnsRecord($type,$name,$value,$ttl,$priority)
        {
            ...
            if($type == "A") $type = "ipv4";
            if($type == "AAAA") $type = "ipv6";
            $type   = strtolower($type);
            if(!$name) $name = $domain;

            $apply = $this->api->add_dns_record($domain,$type,$name,$value,$ttl);
            if(!$apply)
            {
                $this->error = $this->api->error;
                return false;
        ...
        public function updateDnsRecord($type='',$name='',$value='',$identity='',$ttl='',$priority='')
        {
            ...
            $apply = $this->api->update_dns_record($domain,$type,$name,$old_value,$value,$ttl);
            if(!$apply)
            {
                $this->error = $this->api->error;
                return false;
        ...
        public function deleteDnsRecord($type='',$name='',$value='',$identity='')
        {
            ...
            $apply      = $this->api->delete_dns_record($domain,$type,$name,$value);
            if (!$apply)
            {
                $this->error = $this->api->error;
                return false;
