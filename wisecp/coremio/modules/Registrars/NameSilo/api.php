    ...
    class NameSilo_Api
    {
        ...
        private function build($nodecode=false){
            ...
            if($result && !$nodecode && !is_numeric($result)){
                ...
                {
                    if(stristr($query,'checkRegisterAvailability'))
                    {
                        $result_new     = [];
                        $result_decode  = Utility::xdecode($result);
                        preg_match('/<available>(.*?)<\/available>/m', $result, $available_matches);
                        preg_match('/<unavailable>(.*?)<\/unavailable>/m', $result, $unavailable_matches);

                        if(isset($available_matches[1]) && $available_matches[1])
                        {
                            $available_matches = $available_matches[1];
                            preg_match_all('/<domain price="(.*?)" premium="(.*?)">(.*?)<\/domain>/m', $available_matches, $matches,PREG_SET_ORDER, 0);
                            if($matches && sizeof($matches) > 0)
                            {
                                foreach($matches AS $match)
                                {
                                    ...
                                    $result_new["request"] = $result_decode["request"];
                                    $result_new["reply"]["code"] = 300;
                                    $result_new["reply"]["detail"] = "success";
                                    $result_new["reply"]["result"][] = [
                                        'domain'    => $dom,
                                        'status'    => "available",
                                        'price'     => $price,
                                        'premium'   => $premium,
                        ...
                        if(isset($unavailable_matches[1]) && $unavailable_matches[1])
                        {
                            $unavailable_matches = $unavailable_matches[1];
                            preg_match_all('/<domain>(.*?)<\/domain>/m', $unavailable_matches, $matches,PREG_SET_ORDER, 0);
                            if($matches && sizeof($matches) > 0)
                            {
                                foreach($matches AS $match)
                                {
        ...
        public function dnsListRecords($domain=''){
            return $this->get("dnsListRecords")->addParam("domain",$domain)->build();
        }

        public function listEmailForwards($domain=''){
            return $this->get("listEmailForwards")->addParam("domain",$domain)->build();
        }

        public function dnsAddRecord($data=[]){
            $sth = $this->get("dnsAddRecord");
            foreach($data AS $key=>$val) $sth->addParam($key,$val);
            return $sth->build();
        }
        public function dnsDeleteRecord($data=[]){
            $sth = $this->get("dnsDeleteRecord");
            foreach($data AS $key=>$val) $sth->addParam($key,$val);
            return $sth->build();
        }
        public function dnsUpdateRecord($data=[]){
            $sth = $this->get("dnsUpdateRecord");
            foreach($data AS $key=>$val) $sth->addParam($key,$val);
            return $sth->build();
        }
        public function domainForward($data=[]){
            $sth = $this->get("domainForward");
            foreach($data AS $key=>$val) $sth->addParam($key,$val);
            return $sth->build();
        }
        public function configureEmailForward($data=[]){
            $sth = $this->get("configureEmailForward");
            foreach($data AS $key=>$val) $sth->addParam($key,$val);
            return $sth->build();
        }
        public function deleteEmailForward($data=[]){
            $sth = $this->get("deleteEmailForward");
            foreach($data AS $key=>$val) $sth->addParam($key,$val);
            return $sth->build();
        }

        public function dnsSecListRecords($domain=''){
            return $this->get("dnsSecListRecords")->addParam("domain",$domain)->build();
        }
        public function dnsSecAddRecord($data=[]){
            $sth = $this->get("dnsSecAddRecord");
            foreach($data AS $key=>$val) $sth->addParam($key,$val);
            return $sth->build();
        }
        public function dnsSecDeleteRecord($data=[]){
            $sth = $this->get("dnsSecDeleteRecord");
            foreach($data AS $key=>$val) $sth->addParam($key,$val);
            return $sth->build();
        }
