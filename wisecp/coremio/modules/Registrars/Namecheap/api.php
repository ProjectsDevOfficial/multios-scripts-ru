<?php
    class Namecheap_Api {
        ...
        private function post($command='',$data=[]){
            $client_ip  = UserManager::GetIP();
            if($client_ip == "UNKNOWN" || !$client_ip) $client_ip = $_SERVER["SERVER_ADDR"];
            ...
            /*
             $this->_query .= "?ApiUser=".urlencode($this->username);
             $this->_query .= "&UserName=".urlencode($this->username);
             $this->_query .= "&ApiKey=".urlencode($this->api_key);
             $this->_query .= "&ClientIp=".urlencode($client_ip);
             $this->_query .= "&Command=".urlencode($command);
             */
        ...
        private function build($nodecode=false){
            ...
            elseif($this->_type == "POST")
                curl_setopt($this->curl, CURLOPT_POSTFIELDS,Utility::jencode($this->_params));
        ...
        public function domains_dns_setHosts($data=[]){
            $request =  $this->get("namecheap.domains.dns.setHosts")
                ->addParam("SLD",$data["SLD"])
                ->addParam("TLD",$data["TLD"]);

            if(isset($data["RequestValues"]) && $data["RequestValues"])
                foreach($data["RequestValues"] AS $r) $this->addParam($r["Key"],$r["Value"]);
