<?php
    ...
    $product_options    = isset($product["options"]) ? Utility::jdecode($product["options"],true) : [];
    $config_options     = $module->config_options($module_data);
    $selected_plan      = isset($module_data["plan"]) ? $module_data["plan"] : '';
    $group_id           = $_POST["server_group_id"] ?? 0;
...
<div class="formcon" id="plans_wrap">
    ...
    <div class="yuzde70">
            ...
            <?php
                if($config_options["plans"])
                {
                    foreach($config_options["plans"] AS $plan){
                        if(Validation::isInt($selected_plan))
                            $is_selected = $plan["id"] == $selected_plan;
                        else
                            $is_selected = $plan["name"] == $selected_plan;
                        ?>
                        <option<?php echo $plan["id"] == $is_selected ? ' selected' : ''; ?> value="<?php echo $plan["name"]; ?>"
        ...
        <?php
            if($config_options["plans_error"]){
                ?>
                <div class="red-info">
                    <div class="padding10"><?php echo $config_options["plans_error"]; ?></div>
                </div>
                <?php
            }
        ?>
