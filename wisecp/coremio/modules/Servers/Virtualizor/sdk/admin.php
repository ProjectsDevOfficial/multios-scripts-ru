...
class Virtualizor_Admin_API {
	...
	function call($path, $data = array(), $post = array(), $cookies = array()){
        ...
        Modules::save_log("Servers","Virtualizor",$path,['data' => $data,'post' => $post],$resp,$resp ? @unserialize($resp) : $err);
	...
	function iprange($page=1, $reslen=50, $post){
		if(empty($post)){
			...
			$path = 'index.php?act=ipranges&ipsearch='.$post['ipsearch'].'&ippoolsearch='.$post['ippoolsearch'].'&lockedsearch='.$post['lockedsearch'].'&ippid='.$post['ippid'].'&page='.$page.'&reslen='.$reslen;
			$ret = $this->call($path, array(), $post);
		}
		return $ret;
	}
	...
	function ippool($page = 1, $reslen = 50, $post = array()){
		if(empty($post)){
			...
			$path = 'index.php?act=ippool&poolname='.$post['poolname'].'&poolgateway='.$post['poolgateway'].'&netmask='.$post['netmask'].'&nameserver='.$post['nameserver'].'&servers_search='.$post['servers_search'].'&page='.$page.'&reslen='.$reslen;
			$res = $this->call($path);
		}
		return $res;
	}
