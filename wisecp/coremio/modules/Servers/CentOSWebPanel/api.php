<?php
    class CentOSWebPanel_Api {
        ...
        private function build($parse=true){
            ...
            if(!$resultx && !$this->error){
                $this->error = $this->error ?: "API: "."The API did not return any data.";
                return false;
            }
            if($parse){
                ...
                if(!$result){
                    $this->error = $this->error ?: "API: "."Incoming data could not be resolved.";
                    $result = false;
                }
                if($result && isset($result["status"]) && $result["status"] == "Error" && isset($result["msj"])){
                    $this->error = $this->error ?: "API: ".$result["msj"];
                    $result = false;
                }
            }
            else
            ...
            Modules::save_log('Servers','CentOSWebPanel',$this->_full_query,$this->_params,htmlentities($resultx),$this->error ? $this->error : $result);
        ...
        public function email_add($data=[]){
            $stmt =  $this->query("email","add");
            foreach($data AS $k=>$v) $stmt->addParam($k,$v);
            return $stmt->build();
        }

        public function email_list($data=[]){
            $stmt =  $this->query("email","list");
            foreach($data AS $k=>$v) $stmt->addParam($k,$v);
            return $stmt->build();
        }

        public function email_udp($data=[]){
            $stmt =  $this->query("email","udp");
            foreach($data AS $k=>$v) $stmt->addParam($k,$v);
            return $stmt->build();
        }

        public function email_del($data=[]){
            $stmt =  $this->query("email","del");
            foreach($data AS $k=>$v) $stmt->addParam($k,$v);
            return $stmt->build();
        }

        public function admindomains_list($data=[])
        {
            $stmt =  $this->query("admindomains","list");
            foreach($data AS $k=>$v) $stmt->addParam($k,$v);
            return $stmt->build();
        }
