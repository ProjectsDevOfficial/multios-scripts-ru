            ...
            <div class="formcon">
                ...
                <div class="yuzde70">
                    ...
                    <?php
                        ...
                        if(stristr($limit,'Sınırsız') || stristr($limit,'Unlimited')) $limit = '0 MB';

                        $used           = substr($panel_data["bandwidth_used"],0,-3);
                        $percent        = Validation::isInt($limit) && $limit > 0 ? Utility::getPercent((int) $used,(int) $limit) : 0;
                        if($percent > 100) $percent = 100;
                    ?>
                    <div style="margin:20px;display:inline-block;text-align: center;">
                        ...
                        <div class="progress-circle progress-<?php echo $percent; ?>"><span><?php echo $percent; ?></span></div>
                        <div class="clear"></div>
                        <h5 style="font-size:16px;"><?php echo $used == '' ? '0 MB' : $used.' MB'; ?> / <?php echo Validation::isInt($limit) && $limit > 0 ? $limit.' MB' : '∞'; ?></h5>
                    </div>
...
?>
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            if($("#el-SonicPanel-item-reseller").prop('checked'))
            ...
            $("#el-SonicPanel-item-package").change(package_change);
            $("#el-SonicPanel-item-package_r").change(package_r_change);
            $("#el-SonicPanel-item-reseller").change(reseller_change);
        ...
        function package_change(){
            let el = $("#el-SonicPanel-item-package");

            if(!$('#el-SonicPanel-item-reseller').prop('checked'))
            {
                let selection = el.val();
                $('.package-details').css("display","none");
                $("#package-normal-"+md5(selection)).css("display","block");
            }

        }
        function package_r_change(){
            let el = $("#el-SonicPanel-item-package_r");
            if($('#el-SonicPanel-item-reseller').prop('checked'))
            {
                let selection = el.val();
                $('.package-details').css("display","none");
                $("#package-reseller-"+md5(selection)).css("display","block");
            }
        }
        function reseller_change(){
            let el = $("#el-SonicPanel-item-reseller");
            if(el.prop('checked'))
                $("#wrap-el-package_r").css("display","block");
            else
                $("#wrap-el-package_r").css("display","none");

            $("#el-SonicPanel-item-package").trigger("change");
            $("#el-SonicPanel-item-package_r").trigger("change");
        }
    </script>
