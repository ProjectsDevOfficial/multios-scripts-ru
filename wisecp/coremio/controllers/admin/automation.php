<?php
    ...
    class Controller extends Controllers
    {
        ...
        private function repair_cronjob_ip(){
            ...
            if(file_exists(CORE_DIR."LICENSE")) FileManager::file_delete(CORE_DIR."LICENSE");

            Helper::Load("Events");

            $event      = Events::isCreated('system','cronjob',0,'license-ip-check',false,0,true);

            if($event) Events::delete($event["id"]);

            echo 'You are redirected...';

            $redirect = $this->AdminCRLink("dashboard");
        ...
        private function update_settings(){
            ...
            $durn                   = Filter::POST("domain-upcoming-renewal-notice");
            $time_to_process        = Filter::POST("time-to-process");


            if(isset($time_to_process["start"]) && isset($time_to_process["end"]))
            {
                $start          = DateManager::strtotime(DateManager::Now("Y-m-d").' '.$time_to_process["start"]);
                $end            = DateManager::strtotime(DateManager::next_date(['day' => 1],"Y-m-d").' '.$time_to_process["end"]);
                $diff           = $end - $start;
                $diff_second    = $diff / 60;
                $diff_hour      = $diff_second / 60;
                $diff           = $diff_hour - 24;

                if($diff < 1)
                    die(Utility::jencode([
                        'status' => "error",
                        'message' => __("admin/automation/error4"),
                    ]));
            }
            ...
            $invoice_cancellation_day   = $invoice_cancellation["day"] ?? 0;
            $order_cancel_day           = $order_cancel["day"] ?? 0;


            if($invoice_cancellation_day > 0 && !$order_cancel["status"])
            {
                echo Utility::jencode([
                    'status' => "error",
                    'message' => __("admin/automation/error3"),
                ]);
                return false;
            }

            if(!$order_cancel["status"] && $invoice_cancellation_day > 0)
            {
                echo Utility::jencode([
                    'status' => "error",
                    'message' => __("admin/automation/error2"),
                ]);
                return false;
            }


            if($order_cancel["status"] && $invoice_cancellation_day > 0 && $order_cancel_day > 0 && $invoice_cancellation_day <= $order_cancel_day)
            {
                echo Utility::jencode([
                    'status' => "error",
                    'message' => __("admin/automation/error1"),
                ]);
                return false;
            }
            ...
            if($ticket_lock["status"] != Config::get("cronjobs/tasks/ticket-lock/status"))
                $config_sets["cronjobs"]["tasks"]["ticket-lock"]["status"] = (boolean) (int) $ticket_lock["status"];
            ...
            if($durn !=  Config::get("cronjobs/tasks/domain-upcoming-renewal-notice/settings"))
                $config_sets["cronjobs"]["tasks"]["domain-upcoming-renewal-notice"]["settings"] = $durn;
        ...
        private function operationMain($operation=''){
            ...
            if($operation == "repair-cronjob-ip" && Admin::isPrivilege("AUTOMATION_SETTINGS")) return $this->repair_cronjob_ip();
