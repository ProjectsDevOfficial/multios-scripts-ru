<?php
    ...
    class Model extends Models
    {
        ...
        public function get_categories($group=0,$kind='',$lang=false){
            $this->db_start();
            $select = implode(",",[
                't1.id',
                't1.parent',
                ...
                't2.options AS options_lang',
                't3.name AS icon',
                '(SELECT GROUP_CONCAT(id) FROM categories WHERE parent=t1.id) AS subcats',
            ]);
            ...
            if($kind === "server" && $group > 0)
                $sth->where("(SELECT id FROM ".$this->pfx."products WHERE (category=t1.id OR FIND_IN_SET(t1.id,categories)OR FIND_IN_SET(category,(SELECT GROUP_CONCAT(id) FROM categories WHERE parent=t1.id))) AND status='active' AND visibility='visible' LIMIT 1)","","","&&");
        ...
        public function get_list_category($id=0,$lang=false){
            $select = implode(",",[
                't1.id',
                't1.kind',
                't2.title',
        ...
        public function get_category($id=0,$lang=false){
            $this->db_start();
            $select = [
                't1.id',
                't1.kind',
        ...
        public function get_sub_categories($parent=0,$kind='',$lang=false,$temp=[]){
            $sth = $this->db->select("t1.id")->from("categories AS t1");
            $sth->join("LEFT","categories_lang AS t2","t2.owner_id=t1.id AND t2.lang='".$lang."'");
            $sth->where("t2.id","IS NOT NULL","","&&");
            $sth->where("t1.type","=","products","&&");
        ...
        public function get_parent_categories($id=0,$kind='',$lang=false,$temp=[]){
