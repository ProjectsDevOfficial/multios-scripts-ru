<?php
    ...
    class Model extends Models
    {
        ...
        public function get_list($category=0,$clang=false,$start=0,$end=1){
            ...
            $sth->order_by("t1.rank ASC,t1.id DESC");
            $sth->limit($start,$end);
        ...
        public function most_read(){
            ...
            $sth->where("type","=","references","&&");
            $sth->where("status","=","active","&&");
            $sth->where("t2.id","IS NOT NULL");
            $sth->order_by("t1.id DESC");
            $sth->limit(Config::get("options/limits/sidebar-references-most-read"));
            return ($sth->build()) ? $sth->fetch_assoc() : false;
        }
