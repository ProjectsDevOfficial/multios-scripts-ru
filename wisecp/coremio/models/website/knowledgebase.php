<?php
    ...
    class Model extends Models
    {
        ...
        public function get_most_popular($lang){
            ...
            $sth->where("t1.status","=","1","&&");
            $sth->where("t2.id","IS NOT NULL","","");
            $sth->group_by("t1.id,t1.visit_count,t1.useful,t1.useless,t2.route,t2.title");
        ...
        public function get_category($id=0,$clang=false){
            $this->db_start();
        ...
        public function get_list($category=0,$clang=false,$start=0,$end=1){
            $this->db_start();
            $select = [
                't1.id',
                't1.visit_count',
            ...
            $sth->group_by("t1.id,t1.visit_count,t1.useful,t1.useless,t2.title,t2.route");
        ...
        public function get_search_list($query='',$clang=false,$start=0,$end=1){
            $this->db_start();
            $select = [
                't1.id',
                't1.visit_count',
        ...
        public function categories($parent=0,$clang=false){
            $select = [
                't1.id',
                't1.parent',
                't2.title',
