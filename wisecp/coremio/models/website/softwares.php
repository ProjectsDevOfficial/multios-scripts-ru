<?php
    ...
    class Model extends Models
    {
        ...
        public function get_total($category=0,$term='',$lang=false){
            $this->db_start();
        ...
        public function get_list($category=0,$term='',$start=0,$end=1,$lang=false){
            $this->db_start();
            $select = implode(",",[
                't1.id',
            ...
            $sth->where("t1.status","=","active","&&");
            $sth->where("t1.visibility","=","visible");
            $sth->group_by("t1.id,t1.override_usrcurrency,t2.route,t2.title,t1.options,t2.options,t3.name");
            if($category!=0){
                $sth->order_by("t1.rank ASC");
            }else{
