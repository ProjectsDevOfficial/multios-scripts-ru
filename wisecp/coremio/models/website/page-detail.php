<?php
    ...
    class Model extends Models
    {
        ...
        public function get_page($id=0,$clang=false){
            $this->db_start();
            ...
            $sth = $this->db->select(implode(",",[
                ...
                '(SELECT name FROM '.$this->pfx.'pictures WHERE owner=CONCAT(\'page_\',t1.type) AND reason=\'header-background\' AND owner_id=t1.id LIMIT 1) AS header_background',
                '(SELECT name FROM '.$this->pfx.'pictures WHERE owner=CONCAT(\'page_\',t1.type) AND reason=\'mockup\' AND owner_id=t1.id LIMIT 1) AS mockup',
                '(SELECT name FROM '.$this->pfx.'pictures WHERE owner=CONCAT(\'page_\',t1.type) AND reason=\'cover\' AND owner_id=t1.id LIMIT 1) AS cover',
            ]))->from("pages AS t1");
            $sth->join("LEFT","pages_lang AS t2","t1.id=t2.owner_id AND t2.lang='{$clang}'");
