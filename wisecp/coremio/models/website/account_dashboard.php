<?php
    ...
    class Model extends Models
    {
        ...
        public function get_activity($user_id=0,$start=false,$end=false){
            $this->db_start();
        ...
        public function get_tickets($user_id=0,$searches=[],$orders=[],$start=0,$end=5){
            ...
            $case .= "END AS rank";
            $sth = $this->db->select("id,service,title,status,cstatus,locked,ctime,lastreply,userunread,".$case)->from("tickets");
        ...
        public function get_products($uid=0,$type='',$pids=''){
            ...
            $case .= "ELSE 4 ";
            $case .= "END AS rank";
            $sth = $this->db->select("id,invoice_id,type,product_id,name,status,period,period_time,amount,amount_cid,DATE_FORMAT(cdate,'".$format_convert." %H:%i') AS cdate,duedate,options,".$case)->from("users_products");
            if($pids != '')
