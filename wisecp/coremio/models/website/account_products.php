<?php
    ...
    class Model extends Models
    {
        ...
        public function get_products($uid=0,$type='',$pids='',$status=''){
            ...
            $case .= "ELSE 4 ";
            $case .= "END AS rank";
            $sth = $this->db->select("id,invoice_id,type,product_id,name,status,period,period_time,amount,amount_cid,DATE_FORMAT(cdate,'".$format_convert." %H:%i') AS cdate,duedate,options,auto_pay,".$case)->from("users_products");

            if($status && in_array($status,['waiting','active','suspended','cancelled','inprocess']))
                $sth->where("status","=",$status,"&&");
            elseif($status == "upcoming30")
            {
                $sth->where("status","=","active","&&");
                $sth->where("period","!=","none","&&");
                $sth->where("(duedate BETWEEN NOW() - INTERVAL 30 DAY AND NOW()) ","","","&&");
            }
        ...
        public function get_products_count($uid=0,$type='',$pids='',$status=''){
            $sth = $this->db->select("COUNT(id) AS total")->from("users_products");
        ...
        public function isCategory($id=0,$lang=false){
        ...
        public function getProanSe($uid=0,$id=0){
            ...
            $return = $sth->build() ? $sth->getAssoc() : false;
            if($return) {
                if(empty($return['module'] ?? '')) $return["module"] = "none";
            }
            return $return;
        }
