<?php
    ...
    class Model extends Models
    {
        ...
        public function reports_statistic($date='',$uid=false){
        ...
        public function get_reports($uid=0,$start=0,$end=1){
        ...
        public function get_reports_total($uid=0){
            $sth = $this->db->select("COUNT(id) AS total")->from("sms_logs");
            $sth->where("user_id","=",$uid,"&&");
            $sth->where("owner_id","=",0,"&&");
            $sth->where("reason","=","send-sms","&&");
            $sth->where("owner","=","international_sms");
            $result = $sth->build() ? $sth->getObject()->total : 0;
            return $result;
        }
