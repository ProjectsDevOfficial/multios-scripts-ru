<?php
    ...
    class Model extends Models
    {
        ...
        public function get_custom_fields($lang=''){
            $stmt   = $this->db->select()->from("users_custom_fields AS t");
            $stmt->where("status","=","active","&&");
            $stmt->where("lang","=",$lang);
            $stmt->order_by("t.rank ASC");
        ...
        public function getAddresses($id=0){
            return AddressManager::getAddresses($id);
        }
        ...
        public function gdpr_request($uid=0)
        {
            $stmt = $this->db->select()->from("users_gdpr_requests");
            $stmt->where("user_id","=",$uid);
            $stmt->order_by("id DESC");
            return $stmt->build() ? $stmt->getAssoc() : false;
        }

        public function create_gdpr_request($data=[])
        {
            return $this->db->insert("users_gdpr_requests",$data) ? $this->db->lastID() : 0;
        }
        public function cancel_gdpr_request($id=0)
        {
            return $this->db->update("users_gdpr_requests",["status" => "cancelled"])->where("id","=",$id)->save();
        }
