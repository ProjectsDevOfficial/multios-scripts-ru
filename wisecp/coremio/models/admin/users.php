<?php
    ...
    class Model extends Models
    {
        ...
        public function get_actions($id=0,$searches=[],$orders=[],$start=0,$end=1){
            ...
            $sth->where("t1.detail","!=","changed-currency-rates","&&");
        ...
        public function get_affiliate_assignment_clients($searches='',$orders=[],$start=0,$end=1)
        {
            $select = implode(",",[
                'id',
                'full_name',
            ...
            $stmt   = $this->db->select($select)->from("users");
            if($searches){
                ...
                if(isset($searches["word"])){
                    ...
                    if($number) $stmt->where("id","=",$number,"||");
                    $stmt->where("full_name","LIKE","%".$word."%","||");
                    $stmt->where("company_name","LIKE","%".$word."%","||");
                    $stmt->where("email","LIKE","%".$word."%");
        ...
        public function get_affiliate_assignment_clients_total($searches=[])
        {
            $stmt   = $this->db->select("id")->from("users");
            if($searches){
        ...
        public function get_affiliate_assignment_orders($searches='',$orders=[],$start=0,$end=1)
        {
            $select     = implode(',',[
                't1.*',
                't3.id AS user_id',
            ...
            $stmt       = $this->db->select($select)->from("users_affiliate_transactions AS t1");
            ...
            if(isset($searches["word"]) && $searches["word"])
            {
                ...
                if($number) $stmt->where("t3.id","=",$number,"||");
                if($number) $stmt->where("t2.id","=",$number,"||");
                $stmt->where("t3.full_name","LIKE","%".$word."%","||");
                $stmt->where("t3.company_name","LIKE","%".$word."%","||");
                $stmt->where("t2.name","LIKE","%".$word."%","||");
                $stmt->where("t3.email","LIKE","%".$word."%");

                $stmt->where(")","","","&&");
            }

            $stmt->where("t1.affiliate_id","!=","0");
            $stmt->group_by("t1.id");
            $stmt->order_by("t1.id DESC");
            $stmt->limit($start,$end);
            return $stmt->build() ? $stmt->fetch_assoc() : [];
        }
        public function get_affiliate_assignment_orders_total($searches=[])
        {
            $stmt       = $this->db->select('t1.id')->from("users_affiliate_transactions AS t1");
            $stmt->join("LEFT","users_products AS t2","t1.order_id=t2.id");
            $stmt->join("LEFT","users AS t3","t3.id=t2.owner_id");
        ...
        public function select_affiliates($search=''){
            $stmt = $this->db->select("aff.id,t1.full_name,t1.company_name")->from("users_affiliates AS aff");
            $stmt->join("LEFT","users AS t1","t1.id=aff.owner_id");
            if($search){
            ...
            $stmt->order_by("aff.id DESC");
            $stmt->limit(0,10);
            return $stmt->build() ? $stmt->fetch_assoc() : false;
        }
        public function select_clients($search=''){
            $stmt = $this->db->select("t1.id,t1.full_name,t1.company_name")->from("users AS t1");
            if($search){
        ...
        public function select_orders($search=''){
            $stmt = $this->db->select("t1.id,t1.name,t1.options")->from("users_products AS t1");
            if($search){

                $number      = Filter::numbers($search);

                $stmt->where("(");

                $stmt->where("t1.id","LIKE",$number,"||");
                $stmt->where("t1.name","LIKE","%".$search."%","||");
                $stmt->where("t1.options","LIKE","%".$search."%","");

                $stmt->where(")","","","&&");
            }
            $stmt->where("FIND_IN_SET(t1.status,'active,inprocess')");
        ...
        public function delete_affiliate_transaction($id=0)
        {
        ...
        public function get_gdpr_requests($searches='',$orders=[],$start=0,$end=10)
        {
            $select     = implode(",",[
                "t1.*",
                "users.full_name",
                "users.company_name",
                "CASE ".
                "WHEN t1.status = 'pending' THEN 0 ".
                "ELSE 1 ".
                "END AS rak",
            ]);

            $stmt       = $this->db->select($select)->from("users_gdpr_requests AS t1");
            $stmt->join("LEFT","users","users.id=t1.user_id");
            if($searches && isset($searches["word"]) && $searches["word"])
            {
                $word = $searches["word"];

                $stmt->where("(");
                $stmt->where("users.full_name","LIKE","%".$word."%","||");
                $stmt->where("users.phone","LIKE","%".$word."%","||");
                $stmt->where("users.email","LIKE","%".$word."%");
                $stmt->where(")","","","&&");

            }
            $stmt->where("users.id IS NOT NULL");
            $stmt->order_by("rak ASC, t1.id DESC");
            $stmt->limit($start,$end);
            return $stmt->build() ? $stmt->fetch_assoc() : [];
        }
        public function get_gdpr_requests_total($searches='')
        {
            $stmt       = $this->db->select("COUNT(t1.id) AS total")->from("users_gdpr_requests AS t1");
            $stmt->join("LEFT","users","users.id=t1.user_id");
            if($searches && isset($searches["word"]) && $searches["word"])
            {
                $word = $searches["word"];
        ...
        public function get_gdpr_downloaders($searches='',$orders=[],$start=0,$end=10)
        {
            $select = implode(",",[
                't1.id',
                't1.ctime',
            ...
            $stmt = $this->db->select($select)->from("users_actions AS t1");
            $stmt->join("LEFT","users AS t2","t1.owner_id=t2.id");
            ...
            $stmt->where("t1.detail","=","GDPR personal data downloaded");
            $stmt->order_by("t1.id DESC");
            $stmt->limit($start,$end);
            return $stmt->build() ? $stmt->fetch_assoc() : [];
        }
        public function get_gdpr_downloaders_total($searches='')
        {
            $stmt = $this->db->select('COUNT(t1.id) AS total')->from("users_actions AS t1");
            $stmt->join("LEFT","users AS t2","t1.owner_id=t2.id");
        ...
        public function get_gdpr_approvers($searches='',$orders=[],$start=0,$end=10)
        {
            $select = implode(",",[
                't1.id',
                'IF(t3.ctime IS NULL,t1.updated_at,t3.ctime) AS ctime',
                'IF(t3.ip IS NULL,t2.ip,t3.ip) AS ip',
                't1.owner_id AS user_id',
                't2.full_name',
                't2.company_name',
            ]);
            $stmt = $this->db->select($select)->from("users_informations AS t1");
            $stmt->join("LEFT","users AS t2","t1.owner_id=t2.id");
            $stmt->join("LEFT","users_actions AS t3","t3.owner_id=t1.owner_id AND t3.detail='contract2-is-approved'");
            ...
            $stmt->where("t1.name","=","contract2","&&");
            $stmt->where("t1.content","=","1");
            $stmt->order_by("t1.updated_at DESC");
            $stmt->limit($start,$end);
            return $stmt->build() ? $stmt->fetch_assoc() : [];
        }
        public function get_gdpr_approvers_total($searches='')
        {
            $stmt = $this->db->select('COUNT(t1.id) AS total')->from("users_informations AS t1");
            $stmt->join("LEFT","users AS t2","t1.owner_id=t2.id");
        ...
        public function get_gdpr_disapprovers($searches='',$orders=[],$start=0,$end=10)
        {
            return [];
        }
        public function get_gdpr_disapprovers_total($searches='',$orders=[],$start=0,$end=10)
        {
            return 0;
        }

        public function delete_gdpr_request($id=0)
        {
            return $this->db->delete("users_gdpr_requests")->where("id","=",$id)->run();
        }

        public function gdpr_request_detail($id=0)
        {
            $stmt = $this->db->select()->from("users_gdpr_requests")->where("id","=",$id);
            return $stmt->build() ? $stmt->getAssoc() : [];
        }

        public function services_count($user_id=0,$active=1)
        {
            $stmt   = $this->db->select("COUNT(id) AS total")->from("users_products");
            if($active == 1)
            {
                $stmt->where("(");
                $stmt->where("status","=","active","||");
        ...
        public function invoice_count($user_id=0)
        {
            $stmt   = $this->db->select("COUNT(id) AS total")->from("invoices");
            $stmt->where("user_id","=",$user_id);
            return $stmt->build() ? $stmt->getObject()->total : 0;
        }

        public function update_gdpr_request($data=[],$id=0)
        {
            return $this->db->update("users_gdpr_requests",$data)->where("id","=",$id)->save();
        }
