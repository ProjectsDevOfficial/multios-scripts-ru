<?php
    ...
    class Model extends Models
    {
        ...
        public function get_category_hosting_products($category=0,$server=[]){
            $ll_lang    = Config::get("general/local");
            //$sd_lang    = Bootstrap::$lang->clang;
            $server_in_groups       = $this->db->select("servers")->from("servers_groups");
            $server_in_groups->where("type","=","hosting","&&");
            $server_in_groups->where("FIND_IN_SET(".$server["id"].",servers)");
            $server_in_groups       = $server_in_groups->build() ? $server_in_groups->fetch_assoc() : [];

            $pre_ids            = [];
            if($server_in_groups)
            {
                foreach($server_in_groups AS $svi)
                    if($svi["servers"]) $pre_ids = array_merge($pre_ids,explode(",",$svi["servers"]));
            }

            $serverIDs  = $this->db->select("id")->from("servers");

            if($pre_ids)
            {
                $pre_ids = implode(",",$pre_ids);
                $serverIDs->where("FIND_IN_SET(id,'".$pre_ids."')","","","||");
            }
            $serverIDs->where("ip","=",$server["ip"]);
        ...
        public function get_category_server_products($category=0,$server=[]){
            ...
            $server_in_groups       = $this->db->select("servers")->from("servers_groups");
            $server_in_groups->where("type","=","server","&&");
            $server_in_groups->where("FIND_IN_SET(".$server["id"].",servers)");
            $server_in_groups       = $server_in_groups->build() ? $server_in_groups->fetch_assoc() : [];
        ...
        public function sync_hosting($domain='',$user='',$server=[]){
            ...
            $stmt->where(")","","","&&");
            $stmt->where("JSON_UNQUOTE(JSON_EXTRACT(options,'$.server_id'))","=",$server['id'],'&&');
        ...
        public function delete_domain($id=0){
            ...
            $del =  $this->db->delete("tldlist")->where("id","=",$id)->run();
            if($del) $this->db->delete("prices")->where("owner","=","tld","&&")->where("owner_id","=",$id)->run();
            return $del;
        }
        ...
        public function delete_shared_server_group($id=0){
            return $this->db->delete("servers_groups")->where("id","=",$id)->run();
        }

        public function get_shared_server_group($id=0){
        ...
        public function insert_shared_server_group($data=[]){
            return $this->db->insert("servers_groups",$data) ? $this->db->lastID() : false;
        }

        public function set_shared_server_group($id=0,$data=[]){
            return $this->db->update("servers_groups",$data)->where("id","=",$id)->save();
        }
        ...
        public function get_shared_server_list($searches='',$orders=[],$start=0,$end=1){
            ...
            $case .= "ELSE 3 ";
            $case .= "END AS rank";
            $sth = $this->db->select("*,$case")->from("servers");
            if($searches){
                if(isset($searches["word"])){
                    $word       = $searches["word"];
                    $sth->where("(");
        ...
        public function get_shared_server_groups_total($searches=[]){

            $sth = $this->db->select("id")->from("servers_groups");
            if($searches){
                if(isset($searches["word"])){
                    $word       = $searches["word"];
                    $sth->where("(");
        ...
        public function get_shared_server_groups($searches='',$orders=[],$start=0,$end=1){
            $sth = $this->db->select()->from("servers_groups");
            if($searches){
                if(isset($searches["word"])){
                    $word       = $searches["word"];
                    $sth->where("(");
        ...
        public function get_addon_categories_total($searches=[]){
            $search     = $searches["word"] ?? '';
            $sth = $this->db->select("t1.id")->from("categories AS t1");
            if($search){
                $sth->where("(");
                $sth->where("t1.id","=",$search,"||");
                $sth->where('',"EXISTS (
                  SELECT 1 FROM {$this->pfx}categories_lang 
                  WHERE owner_id = t1.id 
        ...
        public function get_addon_categories($searches=[],$orders=[],$start=0,$end=1){
            ...
            $select     = implode(",",[
                "t1.id",
                "COALESCE(
                (SELECT title FROM {$this->pfx}categories_lang WHERE owner_id = t1.id AND lang = '{$lang}' LIMIT 1),
                (SELECT title FROM {$this->pfx}categories_lang WHERE owner_id = t1.id LIMIT 1)) AS name",
            ]);
        ...
        public function get_requirement_categories($searches=[],$orders=[],$start=0,$end=10){
            $lang    = Bootstrap::$lang->clang;
            $search     = $searches["word"] ?? '';
            $select = implode(",",[
                "t1.id",
        ...
        public function get_products($type='',$searches='',$orders=[],$start=0,$end=1,$type_id=0){
            ...
            $select = implode(",",[
                't1.id',
                't1.status',
                'MAX(t2.title) AS name',
                't1.category AS category_id',
                'MAX(t3.title) AS category',
                'MAX(t3.route) AS category_route',
                'MAX(t4.period) AS period',
                'MAX(t4.time) AS time',
                'MAX(t4.amount) AS amount',
                'MAX(t4.cid) AS cid',
                $case,
            ]);
            if($type == "software"){
            ...
            $sth->group_by("t1.id, t1.status, t1.category");
        ...
        public function get_categories_total($type='',$searches=[],$kind_id=0){
            ...
            $sth = $this->db->select("t1.id")->from("categories AS t1");
            $sth->join("LEFT","categories_lang AS t2","t2.owner_id=t1.id AND (t2.lang='".$sd_lang."')");
            $sth->join("LEFT","categories_lang AS t3","t3.owner_id=t1.parent AND (t3.lang='".$sd_lang."')");
            if($searches){
                if(isset($searches["word"])){
                    $word       = $searches["word"];
                    $sth->where("(");
        ...
        public function delete_category($id){
            ...
            $this->db->delete("categories")->where("id","=",$id)->run();
            $this->db->delete("categories_lang")->where("owner_id","=",$id)->run();
        ...
        public function sales_statistics_orders($status='',$type='',$id=0,$wh='this-month')
        {
            $stmt   = $this->db->select("COUNT(id) AS total_count,SUM(total_amount) AS total_fee,amount_cid AS currency");
            $stmt->from("users_products");
            ...
            if($wh == "this-month")
                $stmt->where("cdate","LIKE","%".DateManager::Now("Y-m-")."%","&&");

            if($wh == "this-year")
                $stmt->where("cdate","LIKE","%".DateManager::Now("Y-")."%","&&");


            $stmt->where("type","=",$type,"&&");
            $stmt->where("product_id","=",$id);

            $stmt->group_by("amount_cid");
            return $stmt->build() ? $stmt->fetch_assoc() : [];

        }
        public function get_order_count($type='hosting',$id=0,$status='')
        {
            $stmt   = $this->db->select("COUNT(id) AS total");
            $stmt->from("users_products");
        ...
        public function select_shared_server_groups($type='')
        {
            $stmt = $this->db->select()->from("servers_groups");
            if($type) $stmt->where("type","=",$type);
            $stmt->order_by("id DESC");
            return $stmt->build() ?  $stmt->fetch_assoc() : [];
        }

        public function find_server_group_in_products($id=0)
        {
            $stmt = $this->db->select("id")->from("products");
            $stmt->where("JSON_UNQUOTE(JSON_EXTRACT(options,'$.server_group_id'))","=",$id);
            return $stmt->build() ? $stmt->getObject()->id : 0;
        }

        public function linked_service_price_for_server($id=0)
        {
            $select = [
                'SUM(total_amount) AS amount',
                'COUNT(id) AS total',
                'amount_cid AS currency',
            ];
            $stmt = $this->db->select(implode(",",$select))->from("users_products");
            $stmt->where("JSON_UNQUOTE(JSON_EXTRACT(options,'$.server_id'))","=",$id,"&&");
        ...
        public function check_server_in_group($id=0)
        {
            $stmt = $this->db->select("name")->from("servers_groups");
            $stmt->where("FIND_IN_SET('".$id."',servers)");
            return $stmt->build() ? $stmt->getObject()->name : false;
        }


        public function get_domain_docs_total($searches=[]){
            $sth = $this->db->select("t1.id")->from("tldlist_docs AS t1");
            if($searches){
                if(isset($searches["word"])){
                    $word       = $searches["word"];
                    $sth->where("(");
                    $sth->where("t1.tld","LIKE",$word,"||");
                    $sth->where("t1.options","LIKE","%".$word."%","||");
                    $sth->where("t1.languages","LIKE","%".$word."%");
                    $sth->where(")");
                }
            }
            $sth->group_by("t1.tld");
        ...
        public function get_domain_docs($searches='',$orders=[],$start=0,$end=1){

            $select = implode(",",[
                't1.*',
            ]);
            $sth = $this->db->select($select)->from("tldlist_docs AS t1");
