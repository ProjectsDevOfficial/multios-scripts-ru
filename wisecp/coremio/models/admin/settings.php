<?php
    ...
    class Model extends Models
    {
        ...
        public function get_api_credentials($searches='',$orders=[],$start=0,$end=1){

            $sth = $this->db->select()->from("api_credentials");
            if($searches){
                if(isset($searches["word"])){
                    $word       = $searches["word"];
                    $number     = Filter::numbers($word);
                    $sth->where("(");
                    if(strlen($number) > 0) $sth->where("id","=",$number,"||");
                    $sth->where("identifier","LIKE","%".$word."%","||");
                    $sth->where("name","LIKE","%".$word."%","||");
                    $sth->where("ips","LIKE","%".$word."%","||");
                    $sth->where("permissions","LIKE","%".$word."%");
                    $sth->where(")");
                }
            }
        ...
        public function get_api_credentials_total($searches=''){
            $sth = $this->db->select("id")->from("api_credentials");
            if($searches){
                if(isset($searches["word"])){
                    $word       = $searches["word"];
                    $number     = Filter::numbers($word);
        ...
        public function get_api_logs($searches='',$orders=[],$start=0,$end=1){
            $select = [
                '*',
                '(SELECT name FROM '.$this->pfx.'api_credentials WHERE id=t1.relid) AS api',
            ];
            $sth = $this->db->select(implode(",",$select))->from("api_logs AS t1");
            if($searches){
                if(isset($searches["word"])){
                    $word       = $searches["word"];
                    $number     = Filter::numbers($word);
                    ...
                    {
                        $sth->where("id","=",$number,"||");
                        $sth->where("relid","=",$number,"||");
                    }
                    $sth->where("action","LIKE","%".$word."%","||");
                    $sth->where("request_header","LIKE","%".$word."%","||");
                    $sth->where("request_body","LIKE","%".$word."%","||");
                    $sth->where("response","LIKE","%".$word."%");
                    $sth->where(")");
                }
            }
            $sth->order_by("id DESC");
        ...
        public function get_api_logs_total($searches=''){
            $sth = $this->db->select("id")->from("api_logs");
            if($searches){
                if(isset($searches["word"])){
                    $word       = $searches["word"];
                    $number     = Filter::numbers($word);
