    ...
    class Model extends Models
    {
        ...
        public function pending_replying_tickets(){
            ...
            if($assigned_tickets_only && !$root_admin)
                $stmt->where("t1.assigned","=",$admin_id,"&&");
            ...
            $stmt->where(")");
            $sort_date = 't1.lastreply DESC';
            if(Config::get("options/ticket-show-first") == 1)
                $sort_date = 't1.lastreply ASC';
            $stmt->order_by("rank ASC,".$sort_date);
            $stmt->limit(5);
            return $stmt->build() ? $stmt->fetch_assoc() : false;
        }
        ...
        public function top_tasks($uid,$is_full_admin,$my_dids){
            $case = "CASE ";
            $case .= "WHEN t1.status = 'waiting' THEN 1 ";
            ...
            if($is_full_admin){
                ...
                if($my_dids)
                {
                    ...
                    {
                        $stmt->where("FIND_IN_SET('".$my_did."',t1.departments)","","","||");
                    }
                }
        ...
        public function smartSearch_orders($term='',$domain=false){
            ...
            $stmt->where("(SELECT id FROM ".$this->pfx."users_products_subscriptions WHERE id=t1.subscription_id AND identifier LIKE '%".str_replace("'","\'",$term)."%' LIMIT 1)",">","0","||");
        ...
        public function smartSearch_orders_addons($term='',$pre_orders=[]){
            ...
            $stmt->group_by("t1.id,t1.type,t1.name,t1.options,t1.status,t2.full_name");
            $stmt->order_by("sort_rank ASC,t1.renewaldate DESC,t1.id DESC");
            $stmt->limit(50);
            return $stmt->build() ? $stmt->fetch_assoc() : [];
        }
        public function smartSearch_invoices($term=''){
            ...
            $stmt->where("t2.number","LIKE","%".$term."%","||");

            $stmt->where("t2.pmethod_msg","LIKE","%".$term."%","||");

            $stmt->where("t1.description","LIKE","%".$term."%");

            $stmt->order_by("sort_rank ASC,t2.id DESC,t2.cdate DESC");
            $stmt->group_by("t1.owner_id,t1.description,t2.id,t2.number,t2.status,t3.full_name,t2.status");
            $stmt->limit(50);
        ...
        public function smartSearch_users($term=''){
            ...
            {
                $identity_users     = [];
                $identity_search    = $this->db->select("owner_id")->from("users_informations");
                $identity_search->where("name","=","identity","&&");
                $identity_search->where("content","LIKE","%".$term."%");
                if($identity_search->build()){
                    foreach($identity_search->fetch_object() AS $i){
                        $identity_users[] = $i->owner_id;
                    }
                }
                if($identity_users) $identity_users = implode(",",$identity_users);
            }
            ...
            if($is_int && $term_int)
            {
                $stmt->where("t1.id","=",$term_int,"||");
                if(isset($identity_users) && $identity_users)
                    $stmt->where("FIND_IN_SET(t1.id,'".$identity_users."')","","","||");
            }
        ...
        public function smartSearch_tickets($term=''){
            ...
            if($assigned_tickets_only && !$root_admin)
                $stmt->where("t2.assigned","=",$admin_id,"&&");
            ...
            $stmt->group_by("t1.owner_id,t2.status,t2.id,t2.title,t3.full_name");
