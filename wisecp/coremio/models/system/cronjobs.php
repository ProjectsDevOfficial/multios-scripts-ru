    ...
    class Model extends Models
    {
        ...
        public function remaining_orders($month='',$thn_month=''){
            ...
            if($thn_month){
                ...
                $stmt->where("period","=","year","||");
                $stmt->where("(");
                $stmt->where("period","=","month","&&");
                $stmt->where("period_time",">","1");
                $stmt->where(")");
                $stmt->where(")","","","&&");
        ...
        public function remaining_addons($month='',$thn_month=''){
            ...
            if($month){
                $stmt->where("(");
                $stmt->where("ad.period","!=","hour","&&");
                $stmt->where("ad.period","!=","year","&&");
            ...
            if($thn_month){
                ...
                $stmt->where("ad.period","=","year","||");
                $stmt->where("(");
                $stmt->where("ad.period","=","month","&&");
                $stmt->where("ad.period_time",">","1");
                $stmt->where(")");
                $stmt->where(")","","","&&");
        ...
        public function remaining_invoices($first=0,$second=0,$third=0){
            ...
            foreach($z AS $k => $y) $stmt->where('DATEDIFF(duedate,\''.$now.'\')',"=",$y,$k == $x ? '' : '||');
            $stmt->where(")");
        ...
        public function delayed_orders($type='',$day=0,$hour=0,$status='',$mode=''){
            ...
            if($type == "order"){
                ...
                if($mode == "terminate"){
                    ...
                    $stmt->where("type","=","hosting","&&");
                    $stmt->where("JSON_UNQUOTE(JSON_EXTRACT(options,'$.config.user'))",'!=','');
                    ...
                    $stmt->where("type","=","server","&&");
                    $stmt->where("JSON_UNQUOTE(JSON_EXTRACT(options,'$.config'))",'!=','');
            ...
            elseif($type == "addon"){
                ...
                $stmt->where('DATEDIFF(\''.$now.'\',duedate)',">=",(int) $day);
                $stmt->where(")","","","&&");
        ...
        public function delayed_orders_sc($type='suspend'){
            ...
            $stmt->where($type."_date",">","1991-01-01","&&");
        ...
        public function delete_notifications($day=0){
            ...
            $stmt->where("(");
            $stmt->where("type","=","error","&&");
            $stmt->where("owner","=","system");
            $stmt->where(")","","","||");

            $stmt->where("(");
            $stmt->where("type","=","warning","&&");
            $stmt->where("owner","=","system");
            $stmt->where(")","","","||");

            $stmt->where("(");
            $stmt->where("type","=","success","&&");
            $stmt->where("owner","=","system");
            $stmt->where(")","","","||");

            $stmt->where("(");
            $stmt->where("type","!=","","&&");
            $stmt->where("owner","=","notification","&&");
            $stmt->where("user_id","!=","0");
            $stmt->where(")","","","||");
        ...
        public function delete_spam_notifications(){
            $stmt   = $this->db->select("id")->from("events");

            $stmt->where("status","=","pending","&&");
            $stmt->where("(");
            $stmt->where("(");
            $stmt->where("type","=","info","&&");
            $stmt->where("owner","=","system");
            ...
            $stmt->where(")");
            $stmt->where(")");
            $count   = $stmt->build() ? $stmt->rowCounter() : 0;
            if($count >= 800)
            {
        ...
        public function delete_events()
        {
            ...
            $delete->where("cdate","<= NOW() - INTERVAL 30 DAY");
            $delete->where(")","","","||");
        ...
        public function upcoming_subscriptions()
        {
            ...
            $rows1      = $this->db->select("sbs.*")->from("users_products_subscriptions AS sbs");
            $rows1->join("LEFT","users_products AS up","up.subscription_id=sbs.id");
            $rows1->where("sbs.status","!=","cancelled","&&");
            $rows1->where("up.id","IS NOT NULL","","&&");
            $rows1->where("DATE_FORMAT(up.duedate,'%Y-%m-%d')","<=",DateManager::Now("Y-m-d"),"&&");
            $rows1->where("DATE_ADD(DATE_FORMAT(up.duedate,'%Y-%m-%d'), INTERVAL 5 DAY)",">=",DateManager::Now("Y-m-d"));
            $rows1->group_by("sbs.id");
            $rows1 = $rows1->build() ? $rows1->fetch_assoc() : [];

            $rows2      = $this->db->select("sbs.*")->from("users_products_subscriptions AS sbs");
            $rows2->join("LEFT","users_products_addons AS up","up.subscription_id=sbs.id");
            $rows2->where("sbs.status","!=","cancelled","&&");
            $rows2->where("up.id","IS NOT NULL","","&&");
            $rows2->where("DATE_FORMAT(up.duedate,'%Y-%m-%d')","<=",DateManager::Now("Y-m-d"),"&&");
            $rows2->where("DATE_ADD(DATE_FORMAT(up.duedate,'%Y-%m-%d'), INTERVAL 5 DAY)",">=",DateManager::Now("Y-m-d"));
            $rows2->group_by("sbs.id");
            $rows2 = $rows2->build() ? $rows2->fetch_assoc() : [];

            $row_merge = $rows2 ? array_merge($rows1,$rows2) : $rows1;
            ...
            if($row_merge)
            {
                foreach($row_merge AS $row)
                {
                    if(!in_array($row["id"],$temp_id))
                    {
                        $rows[] = $row;
                        $temp_id[] = $row["id"];
                    }
        ...
        public function clear_module_logs()
        {
            $stmt = $this->db->delete("users_actions");
            $stmt->where("reason","=","module-log","&&");
            $stmt->where("ctime","< NOW() - INTERVAL 15 DAY");
            $stmt->run();
        }
