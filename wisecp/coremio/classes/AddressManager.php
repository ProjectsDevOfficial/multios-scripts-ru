...
NNN NNN',
        ...
        static function getAddresses($id=0){
        ...
        static function getCountryList($lang=''){
            ...
            $sth->where("t2.id","IS NOT NULL");
            $sth->order_by("t1.id ASC");
            $result = $sth->build() ? $sth->fetch_assoc() : false;
            if(!$result && $lang != "en") $result = self::getCountryList("en");
            return $result;
        }
        ...
        static function getAddressesList($id=0,$name='')
        {
            $lang = Bootstrap::$lang->clang;
            $data = self::getAddresses($id);
            if($data){
                ...
                for($i=0;$i<=$size;$i++){
                    ...
                    if(Utility::strlen($var["full_name"]) > 1) $name = $var["full_name"];
                    if(Utility::strlen($var["company_name"]) > 1) $name = $var["company_name"];


                    if(Validation::isInt($var["counti"])){
                        $counti = self::getCountiName($var["counti"]);
                        if($counti)
                            $vname .= " / ".$counti;
                        else
                            $vname .= " / ".$var["counti"];
                        if($counti) $data[$keys[$i]]["counti_name"] = $counti;
                    }elseif($var["counti"] != '' && !Validation::isInt($var["counti"])){
                        $vname .= " / ".$var["counti"];
                        $data[$keys[$i]]["counti_name"] = $var["counti"];
                    }

                    if(Validation::isInt($var["city"])){
                        $city = self::getCityName($var["city"]);
                        if($city)
                            $vname .= " / ".$city;
                        else
                            $vname .= " / ".$var["city"];

                        if($city) $data[$keys[$i]]["city_name"] = $city;
                    }elseif($var["city"] != '' && !Validation::isInt($var["city"])){
                        $vname .= " / ".$var["city"];
                        $data[$keys[$i]]["city_name"] = $var["city"];
                    }

                    if($var["country_id"] != 0){
                        $country = self::get_country_name(self::get_cc_with_id($var["country_id"]),$lang);
                        if($country) $vname .= " / ".$country;
                        $data[$keys[$i]]["country_name"] = $country;
                    }

                    $vname .= " ";
                    if($var["zipcode"] != '') $vname .= " - ".$var["zipcode"];
                    $data[$keys[$i]]["name"] = $name;
                    $data[$keys[$i]]["address_line"] = $vname;
